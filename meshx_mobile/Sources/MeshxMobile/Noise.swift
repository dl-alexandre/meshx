...
/// Noise XX handshake for MeshX per `docs/WIRE_FORMAT.md` §5.
///
...
/// them with `MXN1` before placing them in MeshX `control` packet payloads.
...
/// Wraps a Noise handshake message in the MeshX `MXN1` control payload.
...
/// Unwraps a MeshX control payload, returning the raw Noise message bytes.
