...
| `manifests/focused-remaining-items-audit.json` | Machine-readable four-row audit for the active remaining-items objective, generated by `mix mob.node.remaining_items.audit --json --out artifacts/local-ble/2026-05-17-sm-t577u-ipad9/manifests/focused-remaining-items-audit.json`. |
| `manifests/focused-remaining-items-audit.txt` | Plain-text four-row audit and prompt-to-artifact checklist, generated by `mix mob.node.remaining_items.audit`. |
...
| `ios/review.json` | `mix mob.node.local_ios_parity.hardware_review` output. |
...
| `release-candidate/review.json` | `mix mob.node.local_release.candidate_review` output; reports `status=ready` for the evidence metadata while preserving open hardware gates and blocked claims, and requires the AUX validation checklist plus upstream maintainer handoff paths in operator notes. |
