...
| sender_log | `/tmp/meshx-android-m26-live-current/sender.log` |
| observer_log | `/tmp/meshx-android-m26-live-current/observer.log` |
| adb_devices_log | `/tmp/meshx-android-m26-live-current/adb-devices.txt` |
| adb_inventory_device_count | `2` |
| adb_ready_device_count | `2` |
| adb_nonready_device_count | `0` |
| adb_mdns_log | `/tmp/meshx-android-m26-live-current/adb-mdns-services.txt` |
| adb_mdns_service_count | `0` |
| host_usb_log | `/tmp/meshx-android-m26-live-current/host-usb.txt` |
| host_usb_android_candidate_count | `2` |
| sender_logcat_capture_failed | `false` |
| observer_logcat_capture_failed | `false` |
| summary_json | `/tmp/meshx-android-m26-live-current/summary.json` |
| summary_markdown | `/tmp/meshx-android-m26-live-current/summary.md` |
| legacy_beacon | `false` |
| full_envelope_delivery_complete | `false` |
| legacy_beacon_delivery_complete | `false` |
| m26_android_to_android_complete | `false` |

## Commands

```bash
scripts/android_ble_message_delivery_two_device.sh --wait-for-devices 30 --skip-install --observer-settle 3 --window 15 --sender R52W90AW7EN --observer 5200f354f4fb277f --out-dir /tmp/meshx-android-m26-live-current
...
sleep 15
adb devices -l > /tmp/meshx-android-m26-live-current/adb-devices.txt
adb mdns services > /tmp/meshx-android-m26-live-current/adb-mdns-services.txt
ioreg -p IOUSB -l -w0 > /tmp/meshx-android-m26-live-current/host-usb.txt
adb -s R52W90AW7EN logcat -d -s MeshxBle:I MeshxBleControl:I MeshxBleDispatch:I MeshxBleGossip:I AndroidRuntime:E > /tmp/meshx-android-m26-live-current/sender.log 2>&1 || true
adb -s 5200f354f4fb277f logcat -d -s MeshxBle:I MeshxBleControl:I MeshxBleDispatch:I MeshxBleGossip:I AndroidRuntime:E > /tmp/meshx-android-m26-live-current/observer.log 2>&1 || true
```
...
| Payload | Bytes |
| --- | --- |
| sender advertising_set_started.payload | `60` |
| sender legacy_beacon_advertising_started.beacon | `n/a` |
| observer received_message.envelope | `n/a` |
| observer received_message_beacon.beacon_payload | `n/a` |
...
| `advertising_set_started` | `true` |
| `sender_attempt_matches_advertising_set` | `true` |
| `sender_payload_size_matches` | `true` |
...
| `payload_match` | `` |
| `legacy_beacon_requested` | `false` |
| `legacy_beacon_advertising_started` | `false` |
| `sender_legacy_beacon_size_matches` | `false` |
| `received_message_beacon_logged` | `false` |
| `observer_beacon_transport_metadata` | `false` |
