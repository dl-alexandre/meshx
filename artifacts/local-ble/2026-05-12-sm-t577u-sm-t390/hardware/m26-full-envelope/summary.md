...
| sender_log | `/tmp/mob-android-m26-live-current/sender.log` |
| observer_log | `/tmp/mob-android-m26-live-current/observer.log` |
| adb_devices_log | `/tmp/mob-android-m26-live-current/adb-devices.txt` |
| adb_inventory_device_count | `2` |
| adb_ready_device_count | `2` |
| adb_nonready_device_count | `0` |
| adb_mdns_log | `/tmp/mob-android-m26-live-current/adb-mdns-services.txt` |
| adb_mdns_service_count | `0` |
| host_usb_log | `/tmp/mob-android-m26-live-current/host-usb.txt` |
| host_usb_android_candidate_count | `2` |
| sender_logcat_capture_failed | `false` |
| observer_logcat_capture_failed | `false` |
| summary_json | `/tmp/mob-android-m26-live-current/summary.json` |
| summary_markdown | `/tmp/mob-android-m26-live-current/summary.md` |
| legacy_beacon | `false` |
...
```bash
scripts/android_ble_message_delivery_two_device.sh --wait-for-devices 30 --skip-install --observer-settle 3 --window 15 --sender R52W90AW7EN --observer 5200f354f4fb277f --out-dir /tmp/mob-android-m26-live-current
...
sleep 15
adb devices -l > /tmp/mob-android-m26-live-current/adb-devices.txt
adb mdns services > /tmp/mob-android-m26-live-current/adb-mdns-services.txt
ioreg -p IOUSB -l -w0 > /tmp/mob-android-m26-live-current/host-usb.txt
adb -s R52W90AW7EN logcat -d -s MobBle:I MobBleControl:I MobBleDispatch:I MobBleGossip:I AndroidRuntime:E > /tmp/mob-android-m26-live-current/sender.log 2>&1 || true
adb -s 5200f354f4fb277f logcat -d -s MobBle:I MobBleControl:I MobBleDispatch:I MobBleGossip:I AndroidRuntime:E > /tmp/mob-android-m26-live-current/observer.log 2>&1 || true
```
