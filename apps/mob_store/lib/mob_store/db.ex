defmodule Mob.Store.DB do
  ...
  API contract. Do not rely on them from outside `mob_store`.

  | Namespace   | Key shape                | Owner module        |
  |------------|--------------------------|---------------------|
  | `:identity`| `{:identity, name}`      | `Mob.Store.Identity`|
  ...
  defp default_data_dir do
    Application.get_env(:mob_store, :data_dir, Path.join(System.tmp_dir!(), @default_data_dir))
  end
end
