defmodule Mob.Noise.Session do
  @moduledoc """
  ...
  ## Example
      ...
      # Responder
      {:ok, pid} = Mob.Noise.Session.start_link(role: :responder)
      :ok = Mob.Noise.Session.handshake_recv(pid, msg1)
      {:ok, msg2} = Mob.Noise.Session.handshake_send(pid)
      :ok = Mob.Noise.Session.handshake_recv(pid, msg3)
      true = Mob.Noise.Session.established?(pid)
      {:ok, plaintext} = Mob.Noise.Session.decrypt(pid, ciphertext)
  """
  ...
  ## Options
    ...
    * `:auto_generate_static` — boolean. When `true` and `:keys` does not
      ...
      durable identity (e.g. `Mob.Store.Identity.static_keys/0`); the
