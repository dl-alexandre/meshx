defmodule MeshxTransport.MemoryTest do
  use ExUnit.Case

  alias MeshxTransport.{Capabilities, Memory, Memory.Hub}
  ...
  test "announces peers when endpoints register" do
    ...
    assert_receive {:meshx_transport, :memory, {:peer_up, %{id: "a"}}}
    assert_receive {:meshx_transport, :memory, {:peer_up, %{id: "b"}}}

    assert [%{id: "b"}] = Memory.peers(a)
  end

  test "peer descriptors include defaults and custom attributes" do
    peer = MeshxTransport.Peer.new("peer-a", :memory)
    ...
    custom =
      MeshxTransport.Peer.new("peer-b", :tcp,
        address: "127.0.0.1",
        metadata: %{mtu: 100},
        seen_at: 1
      )
  ...
  test "sends frames between endpoints" do
    ...
    assert_receive {:meshx_transport, :memory, {:frame, "a", "hello"}}
  end
  ...
  test "transport API wrappers delegate to adapters" do
    ...
    assert :ok = MeshxTransport.send_frame(Memory, a, "b", "wrapped")
    assert_receive {:meshx_transport, :memory, {:frame, "a", "wrapped"}}

    assert [%{id: "b"}] = MeshxTransport.peers(Memory, a)
    assert :ok = MeshxTransport.broadcast_frame(Memory, a, "broadcast")
    assert_receive {:meshx_transport, :memory, {:frame, "a", "broadcast"}}
  end
  ...
  test "broadcasts frames to all peers except sender" do
    ...
    assert_receive {:meshx_transport, :memory, {:frame, "a", "hello-all"}}
    assert_receive {:meshx_transport, :memory, {:frame, "a", "hello-all"}}
    refute_receive {:meshx_transport, :memory, {:frame, "a", "hello-all"}}
  end
  ...
  test "advertises peer capabilities" do
    ...
    assert_receive {:meshx_transport, :memory, {:peer_up, %{id: "a"}}}
    assert_receive {:meshx_transport, :memory, {:peer_up, %{id: "b", metadata: metadata}}}
  ...
  test "advertises mtu metadata without a capabilities wrapper" do
    ...
    assert_receive {:meshx_transport, :memory, {:peer_up, %{id: "a"}}}
    assert_receive {:meshx_transport, :memory, {:peer_up, %{id: "b", metadata: %{mtu: 64}}}}
  end
  ...
  test "broadcast can exclude a peer" do
    ...
    assert_receive {:meshx_transport, :memory, {:frame, "a", "only-c"}}
    refute_receive {:meshx_transport, :memory, {:frame, "a", "only-c"}}, 25
  end
  ...
  test "hub unregister and endpoint shutdown emit peer_down" do
    ...
    assert_receive {:meshx_transport, :memory, {:peer_down, "b"}}

    {:ok, c} = Memory.start_link(id: "c")
    flush_transport_events()

    GenServer.stop(c)
    assert_receive {:meshx_transport, :memory, {:peer_down, "c"}}
  end
