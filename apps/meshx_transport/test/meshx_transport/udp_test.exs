defmodule MeshxTransport.UDPTest do
  use ExUnit.Case

  alias MeshxTransport.{Capabilities, UDP}
  ...
  test "two endpoints learn about each other after a hello exchange" do
    ...
    assert_receive {:meshx_transport, :udp, {:peer_up, %{id: "b"} = peer_b}}, 2_000
    assert_receive {:meshx_transport, :udp, {:peer_up, %{id: "a"} = _peer_a}}, 2_000
  ...
  test "frames flow in both directions over UDP" do
    ...
    assert_receive {:meshx_transport, :udp, {:frame, "a", "ping"}}, 2_000

    assert :ok = UDP.send_frame(b, "a", "pong")
    assert_receive {:meshx_transport, :udp, {:frame, "b", "pong"}}, 2_000
  end
  ...
  test "broadcasts frames to every connected peer" do
    ...
    assert_receive {:meshx_transport, :udp, {:frame, "a", "hello-all"}}, 2_000
    assert_receive {:meshx_transport, :udp, {:frame, "a", "hello-all"}}, 2_000
    refute_receive {:meshx_transport, :udp, {:frame, "a", "hello-all"}}, 25
  end
  ...
  test "garbage datagrams are silently dropped" do
    ...
    refute_receive {:meshx_transport, :udp, _}, 200
  ...
  test "frames from endpoints that have not completed hello are dropped" do
    ...
    :ok =
      :gen_udp.send(
        ...
        @localhost,
        port,
        :erlang.term_to_binary({:meshx_udp_frame_v1, "intruder", "payload"})
      )
  ...
  test "keepalive from a rebound peer updates the reverse address lookup" do
    ...
    :ok =
      :gen_udp.send(
        ...
        @localhost,
        port,
        :erlang.term_to_binary({:meshx_udp_hello_v1, "peer", %{}})
      )
    ...
    :ok =
      :gen_udp.send(
        ...
        @localhost,
        port,
        :erlang.term_to_binary({:meshx_udp_keepalive_v1, "peer"})
      )
  ...
  defp assert_peer_up(peer_id) do
    assert_receive {:meshx_transport, :udp, {:peer_up, %{id: ^peer_id} = peer}}, 2_000
    peer
  end

  defp assert_peer_down(peer_id) do
    assert_receive {:meshx_transport, :udp, {:peer_down, ^peer_id}}, 2_000
  end
