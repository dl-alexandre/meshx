defmodule MeshxRuntime.DiscoveryTest do
  use ExUnit.Case

  alias MeshxRuntime.{Discovery, MDNS, PeerRegistry, Router}
  ...
  test "forwards UDP announcements into router peer discovery" do
    ...
    assert_receive {:meshx_runtime, :peer_up, :discovery, peer}
  ...
  test "forwards mDNS announcements into router peer discovery" do
    {:ok, discovery} =
      Discovery.start_link(
        name: :meshx_test_mdns_discovery,
        enabled?: true,
        mdns?: true,
        mdns_port: 0,
        mdns_join?: false,
  ...
  test "ignores self announcements" do
    {:ok, beacon} =
      Discovery.start_link(
        name: :meshx_test_self_discovery,
        enabled?: true,
        id: "local",
        router: Router,
        listen_port: 0,
    ...
    refute_receive {:meshx_runtime, :peer_up, :discovery, _peer}, 50
  ...
  test "announces over UDP and emits telemetry" do
    attach_telemetry([[:meshx_runtime, :discovery, :announce]])

    {:ok, beacon} =
      Discovery.start_link(
        name: :meshx_test_announce_discovery,
        enabled?: true,
        id: "local",
        router: Router,
        listen_port: 0,
    ...
    assert_receive {:telemetry, [:meshx_runtime, :discovery, :announce], %{bytes: bytes},
  ...
  test "uses persistent identity when no explicit id is provided" do
    {:ok, beacon} =
      Discovery.start_link(
        name: :meshx_test_identity_discovery,
        enabled?: true,
        router: self(),
        listen_port: 0,
        target_port: :self,
  ...
  test "emits telemetry for invalid announcements" do
    attach_telemetry([
      [:meshx_runtime, :discovery, :decode_error],
      [:meshx_runtime, :discovery, :peer, :up]
    ])

    {:ok, beacon} =
      Discovery.start_link(
        name: :meshx_test_invalid_discovery,
        enabled?: true,
        id: "local",
        router: Router,
        listen_port: 0,
    ...
    assert_receive {:telemetry, [:meshx_runtime, :discovery, :decode_error], %{count: 1}, _}
    ...
    assert_receive {:telemetry, [:meshx_runtime, :discovery, :peer, :up], %{count: 1}, _}
