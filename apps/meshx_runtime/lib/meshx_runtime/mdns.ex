defmodule MeshxRuntime.MDNS do
  @moduledoc """
  Minimal mDNS/DNS-SD packet support for MeshX LAN discovery.

  MeshX advertises `_meshx._udp.local` with PTR, SRV, TXT, and optional A
  records. TXT records carry the stable peer id, transport, address, and
  metadata needed to build `MeshxTransport.Peer` values.
  """
  ...
  defp instance_name(node_id), do: "meshx-" <> encoded_id(node_id) <> "." <> @service
  defp host_name(node_id), do: "meshx-" <> encoded_id(node_id) <> ".local"
