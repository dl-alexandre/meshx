defmodule Mob.Runtime.Router do
  @moduledoc """
  ...
  @impl true
  def handle_info({:mob_routing, transport, {:peer_up, peer}}, state) do
    ...
    notify(state, {:mob_runtime, :peer_up, transport, peer})
    {:noreply, state}
  end

  def handle_info({:mob_routing, transport, {:peer_down, peer_id}}, state) do
    ...
    notify(state, {:mob_runtime, :peer_down, transport, peer_id})
    {:noreply, state}
  end

  def handle_info({:mob_discovery, {:peer_up, peer}}, state) do
    :ok = PeerRegistry.up(peer)
    ...
    notify(state, {:mob_runtime, :peer_up, :discovery, peer})
    {:noreply, state}
  end

  def handle_info({:mob_routing, transport, {:frame, peer_id, frame}}, state) do
  ...
  defp handle_frame(transport, peer_id, frame, state) do
    case Codec.decode_packet(frame) do
      ...
      {:error, reason} ->
        ...
        notify(state, {:mob_runtime, :decode_error, transport, peer_id, reason})
        state
    end
  end
  ...
  defp handle_application_packet(transport, peer_id, packet, state, relay?) do
    ...
    with {:ok, packet} <- maybe_decrypt_packet(peer_id, packet) do
      ...
      {:error, reason} ->
        ...
        notify(state, {:mob_runtime, :decrypt_error, transport, peer_id, reason})
        state
    end
  end
  ...
  defp route_application_packet(transport, peer_id, packet, state, relay?) do
    ...
    if Dedupe.record?(packet.msg_id) do
      ...
      notify(state, {:mob_runtime, :duplicate, transport, peer_id, packet.msg_id})
      state
    else
      RelayCache.add(packet.msg_id, packet.payload, 0)
      ...
      notify_packet(state, packet, {:mob_runtime, :packet, transport, peer_id, packet})
  ...
  defp maybe_handle_ack_packet(transport, peer_id, %Packet{type: :ack} = packet, state) do
    case Ack.decode_receipt(packet) do
      {:ok, %{kind: :delivery, acked_msg_id: acked_msg_id} = receipt} ->
        ...
        notify(state, {:mob_runtime, :ack, transport, peer_id, acked_msg_id, result})
        notify(state, {:mob_runtime, :delivery_ack, transport, peer_id, acked_msg_id, result})
      ...
      {:ok, %{kind: :read, acked_msg_id: acked_msg_id} = receipt} ->
        ...
        emit_receipt_received(state, transport, peer_id, receipt, result)
        notify(state, {:mob_runtime, :read_receipt, transport, peer_id, acked_msg_id, result})
        {:state, release_flow(peer_id, acked_msg_id, state)}
      ...
      {:error, reason} ->
        ...
        notify(state, {:mob_runtime, :ack_error, transport, peer_id, reason})
        {:state, state}
    end
  end
  ...
  defp emit_receipt_received(state, transport, peer_id, receipt, result) do
    ...
    notify(state, {:mob_runtime, :receipt, transport, peer_id, receipt, result})
  end
  ...
  defp maybe_handle_fragment_packet(
    ...
    case FragmentBuffer.add(packet) do
      {:complete, original_id, frame} ->
        ...
        notify(state, {:mob_runtime, :fragments_complete, transport, peer_id, original_id})
      ...
      {:partial, received, total} ->
        ...
        notify(
          state,
          {:mob_runtime, :fragment, transport, peer_id, packet.msg_id, received, total}
        )
      ...
      {:error, reason} ->
        ...
        notify(state, {:mob_runtime, :fragment_error, transport, peer_id, reason})
        :ok
    end
  end
    ...
    case SessionManager.handle_handshake(peer_id, handshake_msg) do
      {:ok, reply, established?} ->
        ...
        if established? do
          ...
          notify(state, {:mob_runtime, :noise_established, transport, peer_id})
        end
      ...
      {:error, reason} ->
        ...
        notify(state, {:mob_runtime, :noise_error, transport, peer_id, reason})
        :ok
    end
  end
  ...
  defp new_state do
    %{
      transports: %{},
      subscribers: %{},
      flow: FlowControl.new(Application.get_env(:mob_runtime, :flow_control, []))
    }
  end
end
