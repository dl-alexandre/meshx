defmodule Mob.Runtime.Discovery do
  @moduledoc """
  Local UDP and mDNS peer discovery.

  Discovery is LAN-scoped and opt-in. It broadcasts a compact MeshX
  announcement and can also advertise `_mob._udp.local` through mDNS/DNS-SD.
  Received announcements are forwarded to `Mob.Runtime.Router` as discovered
  peers.
  """

  use GenServer

  alias Mob.Runtime.Telemetry
  alias Mob.Runtime.MDNS
  alias Mob.Store.Identity
  alias Mob.Routing.Peer

  @tag :mob_discovery_v1
  @default_port 45_862
  ...
  defp handle_payload(ip, port, payload, state) do
    case decode_announcement(payload) do
      ...
      {:ok, announcement} ->
        ...
        send(state.router, {:mob_discovery, {:peer_up, peer}})
      ...
      {:announcements, announcements} ->
        Enum.reduce(announcements, state, fn
          ...
          announcement, acc ->
            ...
            send(acc.router, {:mob_discovery, {:peer_up, peer}})
