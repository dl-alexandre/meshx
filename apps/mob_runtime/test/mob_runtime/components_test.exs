defmodule Mob.Runtime.ComponentsTest do
  use ExUnit.Case

  @moduletag capture_log: true

  alias Mob.Protocol.Packet
  alias Mob.Runtime.{FragmentBuffer, Outbox, PeerRegistry, Router, SessionManager, Telemetry}
  alias Mob.Store.{Dedupe, Identity, RelayCache, Trust}
  alias Mob.Store.Outbox, as: StoreOutbox
  alias Mob.Routing.{Capabilities, Peer}
  ...
  test "telemetry helper emits under the runtime prefix" do
    attach_telemetry([[:mob_runtime, :custom, :event]])

    assert :ok = Telemetry.execute([:custom, :event], %{count: 1}, source: :test)

    assert_receive {:telemetry, [:mob_runtime, :custom, :event], %{count: 1}, %{source: :test}}
  end
  ...
  test "session manager authorizes established peers and exposes remote keys" do
    ...
    :ok = Mob.Noise.Session.handshake_recv(remote_session, first_message)
    {:ok, second_message} = Mob.Noise.Session.handshake_send(remote_session)

    assert {:ok, third_message, true} = SessionManager.handle_handshake("peer-a", second_message)
    :ok = Mob.Noise.Session.handshake_recv(remote_session, third_message)
  ...
  test "session manager emits Noise telemetry" do
    attach_telemetry([
      [:mob_runtime, :noise, :session, :started],
      [:mob_runtime, :noise, :handshake, :started],
      [:mob_runtime, :noise, :handshake, :established]
    ])

    {:ok, first_message} = SessionManager.ensure_initiator("peer-a")
    {:ok, remote_session} = Mob.Noise.Session.start_link(role: :responder)

    assert_receive {:telemetry, [:mob_runtime, :noise, :session, :started], %{count: 1},
                    %{role: :initiator}}

    assert_receive {:telemetry, [:mob_runtime, :noise, :handshake, :started], %{count: 1},
    ...
    assert_receive {:telemetry, [:mob_runtime, :noise, :handshake, :established], %{count: 1},
                    %{peer_id: "peer-a", role: :initiator}}
  end

  test "session manager rejects key changes under pinned trust" do
    Application.put_env(:mob_store, :trust_policy, :pinned)
  ...
  test "runtime outbox returns encode errors and fails malformed stored rows" do
    attach_telemetry([
      [:mob_runtime, :outbox, :retry, :start],
      [:mob_runtime, :outbox, :replay, :error]
    ])
    ...
    assert_receive {:telemetry, [:mob_runtime, :outbox, :replay, :error], %{count: 1},
                    %{peer_id: "peer-a", msg_id: 100}}

    assert :ok = Outbox.retry_now()
    assert_receive {:telemetry, [:mob_runtime, :outbox, :retry, :start], %{count: 1}, _metadata}
  end
