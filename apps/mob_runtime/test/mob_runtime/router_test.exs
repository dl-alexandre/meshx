defmodule Mob.Runtime.RouterTest do
  use ExUnit.Case

  @moduletag capture_log: true

  alias Mob.Protocol.{Ack, Codec, Fragment, Packet}
  ...
  alias Mob.Routing.{Memory, Memory.Hub, Peer}
  ...
  test "decodes inbound frames, dedupes, stores relay payloads, and emits packet events", %{
    ...
    assert_receive {:mob_runtime, :packet, :memory, "remote", received}
  ...
  test "channel-scoped subscriber only receives matching channels", %{remote: remote} do
    ...
    assert_receive {:mob_runtime, :packet, :memory, "remote", %{channel_id: "chat"}}
    ...
    refute_receive {:mob_runtime, :packet, :memory, "remote", _}, 100
  end
  ...
  test "drops duplicate inbound frames", %{remote: remote} do
    ...
    assert :ok = Memory.send_frame(remote, "local", frame)
    assert_receive {:mob_runtime, :packet, :memory, "remote", _packet}

    assert :ok = Memory.send_frame(remote, "local", frame)
    assert_receive {:mob_runtime, :duplicate, :memory, "remote", ^id}
  ...
  test "sends direct packets to a discovered peer" do
    attach_telemetry([
      [:mob_runtime, :router, :send, :start],
      [:mob_runtime, :router, :send, :stop]
    ])
    ...
    assert_receive {:telemetry, [:mob_runtime, :router, :send, :start], %{frames: 1},
                    %{peer_id: "remote"}}

    assert_receive {:telemetry, [:mob_runtime, :router, :send, :stop], %{frames: 1},
                    %{peer_id: "remote"}}
  end
  ...
  test "unsubscribe suppresses runtime notifications while peer registry still updates", %{
    ...
    refute_receive {:mob_runtime, :peer_down, :memory, "remote"}, 25
  end

  test "emits decode errors for invalid inbound frames", %{remote: remote} do
    attach_telemetry([[:mob_runtime, :router, :frame, :decode_error]])
    flush_transport_events()

    assert :ok = Memory.send_frame(remote, "local", "not-a-packet")
    assert_receive {:mob_runtime, :decode_error, :memory, "remote", _reason}

    assert_receive {:telemetry, [:mob_runtime, :router, :frame, :decode_error], %{count: 1},
                    %{transport: :memory, peer_id: "remote"}}
  end

  test "emits ack errors for malformed ack packets", %{remote: remote} do
    attach_telemetry([[:mob_runtime, :router, :ack, :error]])
    flush_transport_events()
    packet = Packet.new(:ack, msg_id(), "bad")
    {:ok, frame} = Codec.encode_packet(packet)

    assert :ok = Memory.send_frame(remote, "local", frame)
    assert_receive {:mob_runtime, :ack_error, :memory, "remote", :malformed_ack}

    assert_receive {:telemetry, [:mob_runtime, :router, :ack, :error], %{count: 1},
                    %{reason: :malformed_ack}}
  end

  test "emits fragment errors for malformed fragment packets", %{remote: remote} do
    attach_telemetry([[:mob_runtime, :router, :fragment, :error]])
    flush_transport_events()
    packet = Packet.new(:fragment, msg_id(), <<1, 2, 3>>)
    {:ok, frame} = Codec.encode_packet(packet)

    assert :ok = Memory.send_frame(remote, "local", frame)
    assert_receive {:mob_runtime, :fragment_error, :memory, "remote", :malformed_fragment}

    assert_receive {:telemetry, [:mob_runtime, :router, :fragment, :error], %{count: 1},
  ...
  test "emits decode errors for reassembled invalid frames", %{remote: remote} do
    ...
    assert :ok = Memory.send_frame(remote, "local", frame)
    assert_receive {:mob_runtime, :fragments_complete, :memory, "remote", ^original_id}
    assert_receive {:mob_runtime, :decode_error, :memory, "remote", _reason}
  end

  test "emits decrypt errors for encrypted packets without sessions", %{remote: remote} do
    attach_telemetry([[:mob_runtime, :router, :packet, :decrypt_error]])
    flush_transport_events()
    ...
    assert_receive {:mob_runtime, :decrypt_error, :memory, "remote", :session_not_found}

    assert_receive {:telemetry, [:mob_runtime, :router, :packet, :decrypt_error], %{count: 1},
  ...
  test "control packets without handshake payload are delivered as application packets", %{
    ...
    attach_telemetry([[:mob_runtime, :router, :packet, :delivered]])
    flush_transport_events()
    id = msg_id()
    ...
    assert_receive {:mob_runtime, :packet, :memory, "remote", delivered}
    assert delivered.type == :control
    assert delivered.msg_id == id
    assert delivered.payload == "plain-control"

    assert_receive {:telemetry, [:mob_runtime, :router, :packet, :delivered], %{count: 1},
  ...
  test "ttl-one packets are delivered but not relayed", %{remote: remote} do
    ...
    refute_receive {:mob_routing, :memory, {:frame, "local", _frame}}, 50
  end
  ...
  test "delivered ack-requested packets generate direct ACKs", %{remote: remote} do
    ...
    assert_receive {:mob_routing, :memory, {:frame, "local", ack_frame}}
    assert_ack_frame(ack_frame, id)
  end

  test "per-peer backpressure queues ACK-tracked sends until ACK release" do
    attach_telemetry([
      [:mob_runtime, :router, :backpressure, :queued],
      [:mob_runtime, :router, :backpressure, :dropped],
      [:mob_runtime, :router, :backpressure, :dequeued]
    ])
    ...
    assert_receive {:mob_routing, :memory, {:frame, "local", frame1}}, 2_000
    ...
    assert_receive {:telemetry, [:mob_runtime, :router, :backpressure, :queued], %{depth: 1},
    ...
    assert_receive {:telemetry, [:mob_runtime, :router, :backpressure, :dropped], %{count: 1},
                    _}

    refute_receive {:mob_routing, :memory, {:frame, "local", _frame}}, 25

    send_ack_from_peer("remote", packet1.msg_id)
    assert_receive {:mob_runtime, :ack, :memory, "remote", acked_id, _result}
    assert acked_id == packet1.msg_id

    assert_receive {:mob_routing, :memory, {:frame, "local", frame2}}
    assert {:ok, decoded2, <<>>} = Codec.decode_packet(frame2)
    assert decoded2.payload == "window-2"

    assert_receive {:telemetry, [:mob_runtime, :router, :backpressure, :dequeued], %{count: 1},
  ...
  test "queues packets for unknown peers and waits for ACK after replay" do
    ...
    assert_receive {:mob_runtime, :peer_up, :memory, %{id: "later"}}
    assert_receive {:mob_routing, :memory, {:frame, "local", frame}}
    assert {:ok, decoded, <<>>} = Codec.decode_packet(frame)
    ...
    assert_receive {:mob_runtime, :ack, :memory, "later", acked_id, {:ok, _record}}
  ...
  test "store true direct sends request delivery ack and retry when ack is absent" do
    ...
    Outbox.retry_now()
    assert_receive {:mob_routing, :memory, {:frame, "local", retry_frame}}
  ...
  test "outbox retries pending rows until ACK or max attempts" do
    ...
    assert_receive {:mob_runtime, :peer_up, :memory, %{id: "retry"}}
    assert_receive {:mob_routing, :memory, {:frame, "local", _frame1}}
    assert_eventually(fn -> StoreOutbox.get(queued.id).attempts == 1 end)

    Outbox.retry_now()
    assert_receive {:mob_routing, :memory, {:frame, "local", _frame2}}
  ...
  test "read receipt packets mark tracked deliveries read and emit runtime event" do
    ...
    assert :ok = Router.send_packet("remote", packet, store: true, max_attempts: 2)
    assert_receive {:mob_routing, :memory, {:frame, "local", _frame}}

    send_read_receipt_from_peer("remote", packet.msg_id)

    assert_receive {:mob_runtime, :read_receipt, :memory, "remote", acked_id, {:ok, read}}
    assert acked_id == packet.msg_id
  ...
  test "reassembles inbound fragments before packet delivery", %{remote: remote} do
    ...
    refute_receive {:mob_runtime, :packet, :memory, "remote", _packet}, 25

    [last] = Enum.take(frames, -1)
    assert :ok = Memory.send_frame(remote, "local", last)

    assert_receive {:mob_runtime, :fragments_complete, :memory, "remote", ^id}
    assert_receive {:mob_runtime, :packet, :memory, "remote", reassembled}
  ...
  test "peer capabilities can require secure sends" do
    ...
    assert_receive {:mob_runtime, :peer_up, :memory, %{id: "secure"}}
  ...
  test "performs Noise handshake and encrypts direct packets", %{remote: remote} do
    flush_transport_events()
    {:ok, remote_session} = Mob.Noise.Session.start_link(role: :responder)

    assert :ok = Router.ensure_secure_session("remote")
    assert_receive {:mob_routing, :memory, {:frame, "local", frame1}}
    msg1 = decode_handshake_frame(frame1)

    :ok = Mob.Noise.Session.handshake_recv(remote_session, msg1)
    {:ok, msg2} = Mob.Noise.Session.handshake_send(remote_session)
    send_handshake_from_remote(remote, msg2)

    assert_receive {:mob_routing, :memory, {:frame, "local", frame3}}, 2_000
    msg3 = decode_handshake_frame(frame3)
    :ok = Mob.Noise.Session.handshake_recv(remote_session, msg3)

    assert_receive {:mob_runtime, :noise_established, :memory, "remote"}, 2_000
    assert SessionManager.established?("remote")
    assert Mob.Noise.Session.established?(remote_session)
    ...
    assert_receive {:mob_routing, :memory, {:frame, "local", encrypted_frame}}, 2_000
    ...
    assert {:ok, "encrypted-direct"} =
             Mob.Noise.Session.decrypt(remote_session, encrypted_packet.payload)
  end
  ...
  test "drops Noise session on peer_down so reconnect renegotiates", %{
    ...
    assert_receive {:mob_runtime, :peer_down, :memory, "remote"}, 1_000
  ...
  test "reports secure handshake already in progress" do
    ...
    assert :ok = Router.ensure_secure_session("remote")
    assert_receive {:mob_routing, :memory, {:frame, "local", _frame1}}, 2_000
  ...
  test "decrypts inbound encrypted packets", %{remote: remote} do
    ...
    id = msg_id()
    {:ok, ciphertext} = Mob.Noise.Session.encrypt(remote_session, "from-remote")
    ...
    assert_receive {:mob_runtime, :packet, :memory, "remote", packet}
  ...
  test "does not relay decrypted secure packets", %{remote: remote} do
    ...
    id = msg_id()
    {:ok, ciphertext} = Mob.Noise.Session.encrypt(remote_session, "private")
  ...
  defp establish_remote_session(remote, remote_session) do
    ...
    :ok = Mob.Noise.Session.handshake_recv(remote_session, decode_handshake_frame(frame1))
    {:ok, msg2} = Mob.Noise.Session.handshake_send(remote_session)
    send_handshake_from_remote(remote, msg2)

    assert_receive {:mob_routing, :memory, {:frame, "local", frame3}}, 2_000
    :ok = Mob.Noise.Session.handshake_recv(remote_session, decode_handshake_frame(frame3))
    assert_receive {:mob_runtime, :noise_established, :memory, "remote"}, 2_000
    :ok
  end
  ...
  defp send_ack_from_peer(peer_id, acked_msg_id) do
    ...
    {:ok, frame} = Codec.encode_packet(packet)
    assert :ok = Mob.Routing.Memory.Hub.deliver(peer_id, "local", frame)
  end
  ...
  defp collect_transport_frames(acc \\ []) do
    receive do
      {:mob_routing, :memory, {:frame, "local", frame}} ->
