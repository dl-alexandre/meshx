defmodule Mob.Runtime.MDNSTest do
  ...
  test "round-trips host, term, long metadata, and IPv6-like addresses" do
    ...
    cases = [
      {"host-peer", :tcp, {"mob.local", 4_040}, %{host?: true}},
  ...
  test "falls back to SRV and A records when address TXT is absent" do
    instance = "mob-fallback.#{@service}"
    host = "mob-fallback.local"
  ...
  test "returns parser errors for malformed resource data" do
    ...
    assert {:error, :invalid_srv} =
             [resource("mob-bad.#{@service}", @type_srv, <<0, 1>>)]
             |> response_packet()
             |> MDNS.decode_packet()

    assert {:error, :invalid_txt} =
             [resource("mob-bad.#{@service}", @type_txt, <<10, "short">>)]
             |> response_packet()
             |> MDNS.decode_packet()
  end

  defp invalid_term_packet do
    instance = "mob-invalid.#{@service}"
