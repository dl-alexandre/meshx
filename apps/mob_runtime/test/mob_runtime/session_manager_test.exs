defmodule Mob.Runtime.SessionManagerTest do
  @moduledoc """
  ...
  - The replacement session has an independent cipher state: encrypting
    ...
    prevent).
  - `[:mob_runtime, :noise, :session, :dropped]` telemetry fires on drop with the
  ...
  alias Mob.Runtime.SessionManager
  ...
  describe "drop -> ensure_initiator — fresh session lifecycle" do
    test "replaces the session pid and re-initiates a handshake" do
      attach_telemetry([
        [:mob_runtime, :noise, :session, :dropped],
      ...
      assert_receive {:telemetry, [:mob_runtime, :noise, :session, :dropped], %{count: 1},
                      %{peer_id: "peer-b", role: :initiator}}
