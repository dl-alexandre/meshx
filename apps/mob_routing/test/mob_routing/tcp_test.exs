defmodule Mob.Routing.TCPTest do
  use ExUnit.Case

  alias Mob.Routing.{Capabilities, TCP}
  ...
  test "sends frames between connected endpoints" do
    ...
    assert_receive {:mob_routing, :tcp, {:frame, "a", "hello"}}
  end
  ...
  test "broadcasts frames to every connected peer" do
    ...
    assert_receive {:mob_routing, :tcp, {:frame, "a", "hello-all"}}
    assert_receive {:mob_routing, :tcp, {:frame, "a", "hello-all"}}
    refute_receive {:mob_routing, :tcp, {:frame, "a", "hello-all"}}, 25
  end
  ...
  defp assert_peer_up(peer_id) do
    assert_receive {:mob_routing, :tcp, {:peer_up, %{id: ^peer_id} = peer}}
    peer
  end

  defp assert_peer_down(peer_id) do
    assert_receive {:mob_routing, :tcp, {:peer_down, ^peer_id}}
  end
