defmodule Mob.Routing.TCPNegativePathTest do
  @moduledoc """
  ...
  @localhost {127, 0, 0, 1}
  @hello_tag :mob_tcp_hello_v1
  @frame_tag :mob_tcp_frame_v1
  ...
  describe "malformed input" do
    test "an unparseable hello payload causes the connection to be closed silently" do
      ...
      assert_receive {:mob_routing, :tcp, {:peer_up, %{id: "server"}}}, 2_000
    end
    ...
    test "a malformed frame after a valid handshake closes that peer only" do
      ...
      assert_receive {:mob_routing, :tcp, {:peer_up, %{id: "rude"}}}, 2_000
      ...
      assert_receive {:mob_routing, :tcp, {:peer_down, "rude"}}, 2_000
      assert Process.alive?(server)
    ...
    test "many simultaneous client disconnects all produce peer_down events" do
      ...
      observed =
        Enum.reduce(1..n, MapSet.new(), fn _, acc ->
          assert_receive {:mob_routing, :tcp, {:peer_down, peer_id}}, 5_000
  ...
  describe "slow receivers" do
    test "server keeps accepting connections while one peer never reads" do
      ...
      assert_receive {:mob_routing, :tcp, {:peer_up, %{id: "slow"}}}, 2_000
      ...
      assert_receive {:mob_routing, :tcp, {:frame, "server", "ping"}}, 2_000
