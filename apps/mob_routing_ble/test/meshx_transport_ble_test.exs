defmodule Mob.Routing.BLETest do
  use ExUnit.Case

  test "starts with no-op bridge" do
    {:ok, transport} = Mob.Routing.BLE.start_link([])

    assert Mob.Routing.BLE.peers(transport) == []
    assert {:error, :not_configured} = Mob.Routing.BLE.send_frame(transport, "peer", "frame")
    assert {:error, :not_configured} = Mob.Routing.BLE.broadcast_frame(transport, "frame")
  end
  ...
  test "normalizes bridge peer and frame events" do
    ...
    assert_receive {:mob_routing, :ble, {:peer_up, %{id: "peer-a", metadata: %{rssi: -40}}}},
                   1_000

    assert [%{id: "peer-a"}] = Mob.Routing.BLE.peers(transport)

    send(transport, {:ble_frame, "peer-a", "abc"})
    assert_receive {:mob_routing, :ble, {:frame, "peer-a", "abc"}}, 1_000

    send(transport, {:ble_peer_down, "peer-a"})
    assert_receive {:mob_routing, :ble, {:peer_down, "peer-a"}}, 1_000
    assert Mob.Routing.BLE.peers(transport) == []
  end

  test "delegates sends to configured bridge modules" do
    {:ok, transport} =
      Mob.Routing.BLE.start_link(bridge: __MODULE__.Bridge, bridge_opts: [owner: self()])

    assert :ok = Mob.Routing.BLE.send_frame(transport, "peer-a", "frame", mtu: 20)
    assert_receive {:bridge_send, "peer-a", "frame", [mtu: 20]}

    assert :ok = Mob.Routing.BLE.broadcast_frame(transport, "broadcast")
    assert_receive {:bridge_broadcast, "broadcast", []}
  end

  test "port bridge sends commands and normalizes native events" do
    {:ok, bridge} =
      Mob.Routing.BLE.PortBridge.start_link(
        command: "/bin/sh",
        args: ["-c", "while IFS= read -r _line; do :; done"],
        event_target: self()
      )

    assert :ok = Mob.Routing.BLE.PortBridge.send_frame(bridge, "peer-a", "frame", mtu: 20)
    assert :ok = Mob.Routing.BLE.PortBridge.send_frame(bridge, "peer-a", "frame")
    assert :ok = Mob.Routing.BLE.PortBridge.broadcast_frame(bridge, "broadcast")
  ...
  test "port bridge can wait for native command acknowledgements" do
    ...
    send_task =
      Task.async(fn ->
        Mob.Routing.BLE.PortBridge.send_frame(bridge, "peer-a", "frame", mtu: 20)
      end)
    ...
    error_task =
      Task.async(fn ->
        Mob.Routing.BLE.PortBridge.broadcast_frame(bridge, "broadcast")
      end)
  ...
  test "port bridge command acknowledgements time out explicitly" do
    ...
    assert {:error, :command_timeout} =
             Mob.Routing.BLE.PortBridge.send_frame(bridge, "peer-a", "frame")
  end

  test "bluez bridge builds platform command arguments" do
    args =
      Mob.Routing.BLE.BluezBridge.command_args(
        adapter: "hci1",
        local_name: "mob-test",
        mtu: 64,
        scan?: false
    ...
    assert option_value(args, "--local-name") == "mob-test"
  ...
  test "bluez bridge delegates through the port bridge" do
    {:ok, bridge} =
      Mob.Routing.BLE.BluezBridge.start_link(
        command: "/bin/sh",
        args: ["-c", "while IFS= read -r _line; do :; done"],
        event_target: self(),
        command_timeout_ms: 500
    ...
    send_task =
      Task.async(fn ->
        Mob.Routing.BLE.BluezBridge.send_frame(bridge, "peer-a", "frame", mtu: 20)
      end)
    ...
    default_opts_task =
      Task.async(fn ->
        Mob.Routing.BLE.BluezBridge.send_frame(bridge, "peer-a", "frame")
      end)
    ...
    broadcast_task =
      Task.async(fn ->
        Mob.Routing.BLE.BluezBridge.broadcast_frame(bridge, "broadcast")
      end)
  ...
  test "bluez bridge exposes a backend health check" do
    assert {:ok, "ok"} =
             Mob.Routing.BLE.BluezBridge.health_check(
               command: "/bin/sh",
               args: ["-c", "printf 'ok\\n'"],
               timeout: 1_000
             )
    ...
    assert {:error, {:exec, _reason}} =
             Mob.Routing.BLE.BluezBridge.health_check(
               command: "/definitely/not/a/mob/command",
               args: [],
               timeout: 1_000
             )
  end
  ...
  test "bundled bluez bridge executable passes its protocol self-test" do
    if python = System.find_executable("python3") do
      script = Path.expand("../priv/bin/mob_bluez_bridge", __DIR__)
  ...
  defp port_event(port, event) do
    {port, {:data, {:eol, Mob.Routing.BLE.PortBridge.encode_term(event)}}}
  end
