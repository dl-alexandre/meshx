...
`Mob.Routing.BLE` is the **MeshX adapter layer** for any BLE bridge that
speaks the stable `{:ble_peer_up, ...}` / `{:ble_peer_down, ...}` /
`{:ble_frame, ...}` contract. It normalizes those into `{:mob_routing, :ble, ...}`
events for the runtime.

## Bridge behaviours

- `Mob.Routing.BLE.Bridge` — the MeshX-named behaviour definition (for
...
- `Mob.Ble.Bridge` — the **canonical authoritative behaviour** (owned by the
  `mob_ble` Hex package, no mob_* runtime dependency). Mobile production
...
The default `Mob.Routing.BLE.NoopBridge` starts in desktop and test
environments but returns `{:error, :not_configured}` for sends. Linux systems
can use `Mob.Routing.BLE.BluezBridge`, which supervises the bundled
`priv/bin/mob_bluez_bridge` executable. That executable owns BlueZ D-Bus
...
(see `Mob.Node.App` and the wiring test for the exact integration).
