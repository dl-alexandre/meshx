defmodule Mob.Routing.BLE.BluezBridge do
  @moduledoc """
  Linux BlueZ bridge for the BLE transport.

  This bridge uses `Mob.Routing.BLE.PortBridge` to supervise the bundled
  `priv/bin/mob_bluez_bridge` executable. The executable talks to BlueZ over
  ...
  alias Mob.Routing.BLE.PortBridge
  ...
  def default_command do
    case :code.priv_dir(:mob_routing_ble) do
      {:error, _reason} ->
        Path.expand("../../priv/bin/mob_bluez_bridge", __DIR__)

      priv_dir ->
        priv_dir
        |> to_string()
        |> Path.join("bin/mob_bluez_bridge")
    end
  end
