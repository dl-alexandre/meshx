...
| MeshX runtime startup | Done | `Mob.Node.App` starts `mob_runtime` and sets a mobile store directory when Mob provides one. |
| Session/UI contract | Done | `Mob.Node.Session` owns scan/advertise/ping state and is covered by ExUnit. |
| Native BLE bridge | Phase 3 (mob_ble default + published) | Legacy `NativeBridge` path remains for transition (opt-out `MOB_BLE_TRANSPORT=0`). Recommended/default path: `mob_ble` plugin + `Mob.Ble.Bridge` / `Mob.Ble.MobileBridge` wired via `Mob.Routing.BLE` in `Mob.Node.App` (see `maybe_start_mob_ble_transport/0`, `MOB_BLE_*` envs, wiring test, and `docs/mob_ble_bridge_migration.md`). `Mob.Ble.SelfTest` also wired. |
...
- `Mob.Node.App` calls `maybe_start_mob_ble_transport/0` on boot (unless `MOB_BLE_TRANSPORT=0`).
- It starts `Mob.Routing.BLE` using `bridge: Mob.Ble.bridge_module()` (i.e. `Mob.Ble.MobileBridge`, the canonical impl of `Mob.Ble.Bridge`).
- Native events flow through `Mob.Ble.Internal.BridgeProtocol.decode/1` → `MobileBridge` → `Mob.Routing.BLE` → canonical `{:mob_routing, :ble, _}` events.
...
**Migration hygiene:** `mob_node/mix.exs` carries an explicit `{:mob_routing_ble, in_umbrella: true}` dep (required for direct `Mob.Routing.BLE` references in App + wiring test). `mob_ble` itself has zero runtime mob_* deps and is Hex-publishable independently. The `Mob.Routing.BLE.Bridge` behaviour is a CONTRACT-SYNC copy of the authoritative `Mob.Ble.Bridge`.
...
> with `--ez mob_ble_fetch_on_beacon true`; combine it with
> `--ez mob_ble_selftest_send false` for receive-only selftest captures.
...
mix mob.provision
mix mob.devices
mix mob.node.deploy_device --device <device-udid>
```
...
`/tmp/mob-android-m26-live-attempt-latest/summary.json`, which records
one ready adb Android device. The latest readiness recheck artifact is
`/tmp/mob-android-m26-readiness-current/summary.json`; it records the
...
`/tmp/mob-android-sender-regression-latest.log` for Device A dispatch
and `/tmp/mob-android-observer-readiness-latest.log` for Android
...
`android/src/main/java/dev/mob/mob/ble/` holds the transport layer:

- `BleEvent` — sealed class mirroring `Mob.Node.BLE.BridgeProtocol`'s
...
- `BleEventSink` — `LogcatEventSink` for `adb logcat -s MobBle`,
...
The Elixir test `test/mob_node/ble/android_wire_format_test.exs`
loads JSON fixtures matching Kotlin's emitter output and decodes them
through `Mob.Node.BLE.BridgeProtocol`, proving the wire contract
