...
// Packet, Frame live in the local mob_node Swift package.
// Under `xcodebuild`, the Xcode project pulls them in via the
// Mob.Node package product (so `import Mob.Node` is needed).
...
#if canImport(Mob.Node)
import Mob.Node
#endif

final class MobNativeBLEBridge: NSObject {
    static let shared = MobNativeBLEBridge()
    ...
    private var localPeerId = "mob-ios"
    ...
    func sendPing(peerId: String, payload: Data) {
        ...
        do {
            ...
            // iOS counterpart to Android MobBleNative.sendFullMxEnvelope.
...
extension MobNativeBLEBridge: BLEClientDelegate {
...
extension MobNativeBLEBridge: BLEPeripheralDelegate {
...
extension MobNativeBLEBridge: FetchGattResponderDelegate {
...
extension MobNativeBLEBridge: MessageAdvertisementObserverDelegate {
...
public func mob_ble_start_scan() {
    DispatchQueue.main.async {
        MobNativeBLEBridge.shared.startScan()
    }
}
...
public func mob_ble_start_advertising(_ localNamePtr: UnsafePointer<CChar>) {
    ...
    DispatchQueue.main.async {
        MobNativeBLEBridge.shared.startAdvertising(localName: localName)
    }
}
...
public func mob_ble_stop() {
    DispatchQueue.main.async {
        MobNativeBLEBridge.shared.stop()
    }
}
...
public func mob_ble_send_ping(
    ...
    DispatchQueue.main.async {
        MobNativeBLEBridge.shared.sendPing(peerId: peerId, payload: payload)
    }
}
