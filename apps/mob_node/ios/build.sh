#!/bin/bash
# ios/build.sh — Build and deploy Mob.Node to iOS simulator.
...
echo "=== Spot-check ==="
ls "$BEAMS_DIR/Elixir.Mob.Node.App.beam"
ls "$BEAMS_DIR/Elixir.Mob.Node.HomeScreen.beam"
...
xcrun -sdk iphonesimulator swiftc \
    ...
    -o "$BUILD_DIR/Mob.Node"

# ── Bundle + install ───────────────────────────────────────────────────────────
echo "=== Building .app bundle ==="
APP="$BUILD_DIR/Mob.Node.app"
rm -rf "$APP"
mkdir -p "$APP"
cp "$BUILD_DIR/Mob.Node" "$APP/"
