// build.zig — Android native build for meshx_mobile_app.
//
...
//     Output: libmeshx_mobile_app.so installed to
//     android/app/src/main/jniLibs/<abi>/libmeshx_mobile_app.so so
...
pub fn build(b: *std.Build) void {
    ...
    // libs + plain libcrypto.a + Android system libs). Installs the .so
    // into jniLibs/. (Return value previously fed exqlite's link edge,
    // which mob_node doesn't have — discarded.)
    _ = addLink(b, native_lib_step, .{
    ...
    // --- exqlite / SQLite NIF ─────────────────────────────────────────────
    // mob_node uses CubDB for persistence and has no exqlite/SQLite NIF.
    // mob_dev passes -Dexqlite_src=<root>/deps/exqlite/c_src unconditionally,
    // but that path does not exist for this project, so there is nothing to
    // compile or link here. Discard the option and skip the SQLite build.
    _ = exqlite_src;
}
