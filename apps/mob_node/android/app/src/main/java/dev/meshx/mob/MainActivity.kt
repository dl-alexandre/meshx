...
class MainActivity : ComponentActivity() {
    ...
    companion object {
        ...
        init { System.loadLibrary("mob_node") }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // code. Mob.Store.DB falls back through System.tmp_dir!/0 during
        ...
        // mob_ble_selftest — when set, Mob.Node.App.on_start runs
        // the headless BLE bring-up probe (Mob.Node.BleSelfTest)
        ...
        // RT-01 reliability event logging — enables Mob.Node.BLE.Observability
        // to emit "MobAppEvent:" timeline lines consumed by
        // mix mob.node.rt01.analyze. Off unless explicitly launched with the
        // extra, so normal runs stay quiet.
        if (intent?.extras?.getBoolean("mob_rt_event_log", false) == true) {
            android.system.Os.setenv("MESHX_RT_EVENT_LOG", "1", true)
            Log.i(TAG, "onCreate: MESHX_RT_EVENT_LOG=1")
        }

        intent?.extras?.getString("mob_rt_run_id")?.takeIf { it.isNotEmpty() }?.let { runId ->
        ...
        // `adb shell dumpsys deviceidle whitelist +dev.mob.mob`.
        if (intent?.extras?.getBoolean("mob_ignore_battery_opt", false) == true) {
        ...
        // MOB_BLE_* env vars consumed by Mob.Node.App and
        ...
        // MOB_BLE_FETCH_ON_BEACON parity (in addition to legacy mob_ name)
        ...
        val fetchOnBeacon = mobBleFetch || (intent?.extras?.getBoolean("mob_ble_fetch_on_beacon", false) == true)
