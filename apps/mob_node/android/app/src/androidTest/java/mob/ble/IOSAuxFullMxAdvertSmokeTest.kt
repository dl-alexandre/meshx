 ...
 *
 * iOS observer (always use this with --mob-log-raw-advert-data for these experiments):
 *   xcrun devicectl device process launch --device <UDID> --terminate-existing --console \
 *     dev.mob.node.harness -- --mob-auto-scan --mob-log-candidate-discoveries \
 *     --mob-log-raw-advert-data
 *
 ...
 *     -- --mob-auto-direct-mx-service-advertise
 ...
 *   xcrun devicectl device process launch --device <UDID> ... \
 *     -- --mob-auto-direct-mx-hybrid-advertise
...
class IOSAuxFullMxAdvertSmokeTest {
    ...
    fun emitsServiceDataFullMxEnvelope() {
        ...
        // Pair this with the iOS observer using --mob-log-raw-advert-data.
        val context: Context = InstrumentationRegistry.getInstrumentation().targetContext
        val manager = context.getSystemService(Context.BLUETOOTH_SERVICE) as BluetoothManager
        val adapter: BluetoothAdapter? = manager.adapter
        ...
        val envelope = MobMessageEnvelope.buildV1(
            messageId = messageId,
            senderPeerId = "mob-android-svcdata",
            recipientPeerId = null,
            createdAtMs = System.currentTimeMillis(),
            ttl = 1,
            payloadType = "TX",
    ...
    fun emitsHybridMbCuePlusServiceDataFullMxEnvelope() {
        ...
        //
        // On iOS with --mob-log-raw-advert-data you should see:
        ...
        val envelope = MobMessageEnvelope.buildV1(
            messageId = messageId,
            senderPeerId = "mob-android-hybrid",
            recipientPeerId = null,
            createdAtMs = System.currentTimeMillis(),
            ttl = 1,
            payloadType = "TX",
    ...
    // )
    //
    // Then launch iOS with --mob-log-raw-advert-data. The raw dump will show the
    ...
    // The iOS raw observer with --mob-log-raw-advert-data + mx_magic_seen will tell us
