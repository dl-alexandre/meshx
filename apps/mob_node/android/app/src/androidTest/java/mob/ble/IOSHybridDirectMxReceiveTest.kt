 ...
 *
 * When the iOS harness is launched with `--mob-auto-direct-mx-hybrid-advertise`, it emits:
 *   - a short legacy MB (22-byte manufacturer data) cue (fleet-compatible beacon containing 8-byte hashes)
 *   - (claimed) the full v1 MobMessageEnvelope carried in service data under MOB_DIRECT_MX_SERVICE_UUID (...1001)
 *
 ...
 *     and its payload is successfully parsed as a full MX envelope via `MobMessageEnvelope.parse`
 ...
 *   - No MobFetchGatt fetch-service (....2000) advertisements were observed during the window
 ...
 * `InMemoryEventSink` + a minimal `MobBeaconFetchCoordinatorHook` recording implementation.
 * No heavy mocks. The same permission set and adapter-guard pattern as `IOSAuxFullMxAdvertSmokeTest`.
 *
 * Permission shim: the `permissions` rule below is version-aware (API >=31 vs <=30) so
 * GrantPermissionRule no longer fails on SM-T390 (Android 9 / API 28). This lets the
 * instrumented tests reach the BLE logic on the full supported fleet. For production
 * scanner confidence on T390, the main-app selftest path (`mob_ble_selftest=true`)
 * is still preferred (see `docs/ble-lab-cheat-sheet.md`).
 *
 ...
 *
 * (Optional but recommended for deeper radio evidence: add --mob-log-raw-advert-data to the iOS side.)
 *
 * Then, while iOS is emitting, run this test on the Android device:
 *
 *   adb shell am instrument -w -e class mob.ble.IOSHybridDirectMxReceiveTest \
...
class IOSHybridDirectMxReceiveTest {
    ...
    fun receivesHybridDirectMxFromIOS() {
        ...
        val fetchHook = object : MobBeaconFetchCoordinatorHook {
        ...
        val detailedDirects = Collections.synchronizedList(mutableListOf<MobMessageEnvelope.Decoded>())
        ...
        val callback = object : ScanCallback() {
            private fun handleResult(result: ScanResult) {
                ...
                if (svcData != null && svcData.size >= 2 &&
                    ...
                    when (val pr = MobMessageEnvelope.parse(svcData)) {
                        is MobMessageEnvelope.ParseResult.Ok -> {
                ...
                val hasFetchSvc = record.serviceUuids?.any {
                    it.uuid == MobFetchGatt.SERVICE_UUID
                } == true
                if (hasFetchSvc) {
        ...
        assertTrue(
            "MB legacy cue (22-byte manufacturer data beacon) must have been received and recognized " +
                "as a ReceivedMessageBeacon (via MobMessageAdvertisement.decodeScanRecord)",
            beacons.isNotEmpty() || productionBeacons.isNotEmpty()
        )

        assertTrue(
            "Direct-MX service-data advertisement on MOB_DIRECT_MX_SERVICE_UUID must have been " +
                "observed and its payload successfully parsed as a full v1 MobMessageEnvelope " +
                "(via MobMessageEnvelope.parse on the raw service data bytes)",
            directs.isNotEmpty()
        )
        ...
        assertTrue(
            "Expected zero MobFetchGatt SERVICE_UUID (....2000) sightings during the direct-MX hybrid " +
