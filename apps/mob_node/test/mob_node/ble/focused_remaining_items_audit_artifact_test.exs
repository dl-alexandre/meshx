defmodule Mob.Node.BLE.FocusedRemainingItemsAuditArtifactTest do
  ...
  test "blocked rows match fresh external blocker source artifacts" do
    ...
    assert "mob_dependency_migration" in migration_progress["missing_criteria"]
  ...
  test "completed startup row matches root mix test log" do
    ...
    protocol_count = startup["observed_state"]["test_counts"]["mob_protocol"]

    for {app, expected} <-
          Map.delete(startup["observed_state"]["test_counts"], "mob_protocol") do
      assert log =~ "==> #{app}"
      assert log =~ "#{expected} tests, 0 failures"
    end

    assert protocol_count == 47
    assert log =~ "==> mob_protocol"
  ...
  test "recent evidence inventory indexes closure artifacts without closing them" do
    ...
    assert "mob_dependency_migration" in items[
  ...
  test "release criteria in local release manifest cite focused and recent evidence gates" do
    ...
    assert "mix mob.node.remaining_items.audit --json --out <path>" in audit_artifacts[
             "evidence"
           ]

    assert "mix mob.node.local_release.recent_evidence --json --out <path>" in audit_artifacts[
             "evidence"
           ]
  ...
  defp repo_path(path) do
    ...
    repo_root =
      if Path.basename(cwd) == "mob_node" do
        Path.expand("../..", cwd)
      else
        cwd
      end
