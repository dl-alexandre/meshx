defmodule Mob.Node.BLE.BeaconFetchRequestTest do
  use ExUnit.Case, async: true

  alias Mob.Node.BLE.Events.ReceivedMessageBeacon
  alias Mob.Node.BLE.{BeaconFetchRequest, BeaconRef, BeaconResolver, MessageEnvelope}
  ...
  test "unknown beacon becomes a deterministic auditable fetch intent" do
    ...
    assert {:ok, request} =
             BeaconFetchRequest.from_resolver_result(resolver_result,
               ...
               candidate_source_peer_ids: ["meshx-alpha", "mob-relay"],
               id_fun: fn request ->
    ...
    assert request.requesting_peer_id == "mob-local"
    assert request.candidate_source_peer_ids == ["meshx-alpha", "mob-relay"]
