defmodule Mob.Node.BLE.LocalReleaseManifestTest do
  ...
  test "manifest lists required release commands and artifacts" do
    ...
    assert Enum.any?(
             ...
             &String.contains?(&1, "mob.node.advert_gossip.audit")
           )
    ...
    assert "mix mob.node.remaining_items.audit --json --out <path>" in manifest.required_commands
    assert "mix mob.node.remaining_items.audit | tee <path>" in manifest.required_commands
    ...
    assert "mix mob.node.local_completion.audit --allow-open" in manifest.required_commands
  ...
  defp artifact_producing_command?("mix mob.node.advert_gossip.audit " <> _rest), do: true
