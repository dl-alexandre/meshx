defmodule Mob.Node.BLE.LocalReleaseCriteriaTest do
  use ExUnit.Case, async: true

  alias Mob.Node.BLE.{LocalInbox, LocalProjectReadiness, LocalReleaseCriteria}
  ...
  test "release audit artifacts are satisfied but do not close readiness blockers" do
    ...
    assert Enum.any?(
             criterion.evidence,
             &String.contains?(&1, "mob.node.local_readiness.audit")
           )
    ...
    assert Enum.any?(
             criterion.evidence,
             &String.contains?(&1, "mob.node.remaining_items.audit")
           )

    assert Enum.any?(
             criterion.evidence,
             &String.contains?(&1, "mob.node.local_release.recent_evidence")
           )
  ...
  test "nearby messages surface includes Mob controls but remains UX-limited" do
    ...
    assert criterion.status == :satisfied
    assert "Mob.Node.HomeScreen" in criterion.evidence
