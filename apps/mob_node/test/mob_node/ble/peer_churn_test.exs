defmodule Mob.Node.BLE.PeerChurnTest do
  use ExUnit.Case, async: true

  alias Mob.Node.BLE.{Events, PeerChurn, PeerInventory, PeerTable, Replay}
  alias Mob.Node.BLE.PeerChurn.ChurnEvent
  ...
  describe "diff/3 — identity events" do
    ...
    test "same name across rotation produces neither :identity_promoted nor :identity_conflict" do
      # Two device_ids advertising the same `mob-alpha` — that's
    ...
    test "competing claim on the same device_id triggers :collision_detected, sticky peer_id means NO :identity_conflict" do
      ...
      # conflicting `mob-beta` claim. The conflict surfaces as
