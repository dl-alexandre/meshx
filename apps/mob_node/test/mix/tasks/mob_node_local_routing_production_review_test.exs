defmodule Mix.Tasks.Mob.NodeLocalRoutingProductionReviewTest do
  use ExUnit.Case, async: false

  import ExUnit.CaptureIO

  alias Mix.Tasks.Mob.Node.LocalRouting.ProductionReview
  alias Mob.Node.BLE.LocalRoutingProductionEvidenceReview

  setup do
    Mix.Task.reenable("mob.node.local_routing.production_review")
  ...
  test "prints an open review summary without input" do
    ...
    assert output =~
             "ROUTING_PRODUCTION_TEMPLATE command=mix mob.node.local_routing.production_review --template"
  end
