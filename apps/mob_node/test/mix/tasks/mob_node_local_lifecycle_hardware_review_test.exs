defmodule Mix.Tasks.Mob.NodeLocalLifecycleHardwareReviewTest do
  use ExUnit.Case, async: false

  import ExUnit.CaptureIO

  alias Mix.Tasks.Mob.Node.LocalLifecycle.HardwareReview
  alias Mob.Node.BLE.LocalLifecycleHardwareEvidenceReview

  setup do
    Mix.Task.reenable("mob.node.local_lifecycle.hardware_review")
  ...
  test "prints an open review summary without input" do
    ...
    assert output =~
             "LIFECYCLE_HARDWARE_TEMPLATE command=mix mob.node.local_lifecycle.hardware_review --template"
  end
