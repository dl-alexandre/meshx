defmodule Mix.Tasks.Mob.NodeLocalReleaseManifestTest do
  use ExUnit.Case, async: false

  import ExUnit.CaptureIO

  alias Mix.Tasks.Mob.Node.LocalRelease.Manifest

  setup do
    Mix.Task.reenable("mob.node.local_release.manifest")
  ...
  test "writes machine-readable JSON artifact" do
    ...
    assert "mix mob.node.local_completion.audit --allow-open" in decoded["required_commands"]
    ...
    assert "mix mob.node.remaining_items.audit --json --out <path>" in decoded[
             "required_commands"
           ]

    assert "mix mob.node.remaining_items.audit | tee <path>" in decoded["required_commands"]
