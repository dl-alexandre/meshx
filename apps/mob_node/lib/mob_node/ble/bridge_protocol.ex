defmodule Mob.Node.BLE.BridgeProtocol do
  @moduledoc """
  ...
  maps into canonical `Mob.Node.BLE.Event` structs. Bridges do no
  ...
  alias Mob.Node.BLE.{Capabilities, Error, MessageAdvertisement, MessageEnvelope}
  ...
  @event_tags %{
    ...
    "error" => Mob.Node.BLE.Events.Error
  }
  ...
  """
  @spec decode(term()) :: {:ok, Mob.Node.BLE.Event.t()} | {:error, term()}
  ...
  # The Android NIF (c_src/mob_ble_nif.c) delivers events as the v1
  ...
  """
  @spec encode(Mob.Node.BLE.Event.t()) :: map()
  ...
  def encode(%Mob.Node.BLE.Events.Error{} = e) do
    %{
      v: @wire_version,
      event: "error",
      kind: Atom.to_string(e.kind),
  ...
  defp decode_v1(tag, msg) do
    case Map.fetch(@event_tags, tag) do
      {:ok, Mob.Node.BLE.Events.Error} -> decode_error(msg)
