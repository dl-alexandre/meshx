defmodule Mob.Node.BLE.MessageAdvertisement do
  @moduledoc """
  ...
  defp manufacturer_payloads(<<_len, rest::binary>>, acc) do
    cond do
      ...
      truncated_mob_ad_structure?(rest) ->
        {:error, :truncated_ad_structure}
  ...
  defp has_message_payload?(entries) do
    Enum.any?(entries, fn
      %{company_identifier: @mob_company_identifier, payload: <<@message_magic, _rest::binary>>} ->
        true

      %{company_identifier: @mob_company_identifier, payload: <<@beacon_magic, _rest::binary>>} ->
        true
  ...
  defp truncated_mob_ad_structure?(
         <<@manufacturer_specific_data, @mob_company_identifier::16-little-unsigned,
           payload::binary>>
       ) do
  ...
  defp truncated_mob_ad_structure?(_rest), do: false

  defp find_message_payload([]), do: :not_message_advertisement

  defp find_message_payload([
         %{
           company_identifier: @mob_company_identifier,
