defmodule Mob.Node.BLE.LocalFocusedRemainingItemsAudit do
  @moduledoc """
  ...
  @rows [
    %{
      ...
      success_criteria: [
        ...
        "Android connects to the iOS MobFetchGattResponder service.",
    ...
    %{
      ...
      evidence: [
        ...
        "apps/mob_node/lib/mob_node/ble/local_ios_advert_carrier_decision.ex",
        "apps/mob_node/lib/mob_node/ble/local_hardware_validation_gates.ex",
    ...
    %{
      ...
      success_criteria: [
        ...
        "Downstream patch files and mix mob.patch_deps requirement are removed.",
      ...
      observed_state: %{
        ...
        ios_swift_sources_config: "present in mob.exs (14 Mob.Node + MobBLEBridge.swift)",
      ...
      remaining_gap:
        "None. MeshX now consumes the upstream :ios_swift_sources and :static_nifs extension points. The two downstream patch files and the mob.patch_deps task + aliases have been removed. See docs/upstream_mob_migration_checklist.md for execution record.",
      completion_claim_allowed: true
    },
    %{
      ...
      success_criteria: [
        ...
        "The prior Mob.Runtime/Mob.Noise.Supervisor startup conflict does not fail umbrella tests.",
        "Runtime and scripts preserve explicit dependency startup behavior."
      ],
      evidence: [
        "apps/mob_runtime/lib/mob_runtime.ex",
      ...
      observed_state: %{
        ...
        test_counts: %{
          ...
          mob_routing_ble: 10,
          mob_runtime: 70,
          # (was meshx_mobile_app: 1265 + meshx_mob: 15 → meshx_mob absorbed into mob_node)
          mob_node: 1280
        }
      },
  ...
  @prompt_to_artifact_checklist [
    %{
      ...
      tests: [
        "apps/mob_node/test/mob_node/ble/local_focused_remaining_items_audit_test.exs",
        "apps/mob_node/test/mob_node/ble/focused_remaining_items_audit_artifact_test.exs",
        "apps/mob_node/test/mix/tasks/mob_node_remaining_items_audit_test.exs"
      ],
      status: :partial,
    ...
    %{
      ...
      commands: [
        ...
        "Android service-data carrier experiment (different advertising strategy) — run emitsServiceDataFullMxEnvelope() in IOSAuxFullMxAdvertSmokeTest (uses MESHX_DIRECT_MX_SERVICE_UUID); pair with iOS observer using --mob-log-raw-advert-data (look for mx_magic_seen=true or [MX_MAGIC] in the raw dump)",
        "Android hybrid experiment (MB legacy cue + service-data full payload) — run emitsHybridMbCuePlusServiceDataFullMxEnvelope() in IOSAuxFullMxAdvertSmokeTest; pair with iOS observer using --mob-log-raw-advert-data (look for mx_magic_seen=true on both the MB cue and the service-data UUID)",
        "iOS direct-MX service-data emit experiment (reverse direction) — iOS harness with --mob-auto-direct-mx-service-advertise (emits full MX on the direct service UUID); pair with Android service-data / raw observer to test iOS → Android on the new carrier",
        "iOS hybrid emit (MB cue + direct service-data full payload) — iOS harness with --mob-auto-direct-mx-hybrid-advertise; the symmetric iOS→Android version of the Android hybrid test. Pair with Android raw observer and look for matching messageId in both the MB beacon and the DIRECT_MX_SERVICE_DATA_WITH_MAGIC log.",
        "iOS observer (with raw dump for all carriers): xcrun devicectl device process launch --device <udid> --terminate-existing --console dev.mob.node.harness -- --mob-auto-scan --mob-log-candidate-discoveries --mob-log-raw-advert-data"
      ],
      tests: [
        "apps/mob_node/test/mob_node/ble/focused_remaining_items_audit_artifact_test.exs",
        "apps/mob_node/test/mob_node/ble/local_hardware_validation_gates_test.exs"
      ],
      status: :blocked,
      gap:
    ...
    %{
      ...
      tests: [
        "apps/mob_node/test/mob_node/ble/focused_remaining_items_audit_artifact_test.exs",
        "apps/mob_node/test/mob_node/ble/local_project_readiness_test.exs"
      ],
      status: :complete,
      gap:
