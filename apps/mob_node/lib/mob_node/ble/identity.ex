defmodule Mob.Node.BLE.Identity do
  @moduledoc """
  ...
  begins with the ASCII prefix `"mob-"`, return the full local name
  as the peer_id.
  ...
  # Accepted peer-name prefixes. "mob-" is the going-forward prefix; "meshx-"
  # is preserved for backward-compat with historical captures + devices that
  # were imaged before the mob_* rename and still advertise with the old name.
  @mob_prefix "mob-"
  @meshx_prefix "meshx-"
  ...
  defmodule Claim do
    ...
    `Mob.Node.BLE.Identity.classify/1`.
  ...
  def classify(advertisement) when is_binary(advertisement) do
    case extract_local_name(advertisement) do
      <<@mob_prefix, _::binary>> = name ->
        %Claim{peer_id: name, source: :advertised_name}

      <<@meshx_prefix, _::binary>> = name ->
        %Claim{peer_id: name, source: :advertised_name}
