defmodule Mob.Node.BleSelfTest do
  @moduledoc """
  Headless on-device BLE bring-up probe.

  Started by `Mob.Node.App.on_start/0` only when the
  `MESHX_BLE_SELFTEST` env var is set (the Android launcher derives it
  from the `mob_ble_selftest` intent extra). It exercises the real
  `mob_ble_nif` path end to end without any UI:
    ...
    2. Receives `{Mob.Node.NativeBridge, :bridge_event, json}` and
       ...
       `mob_ble` path (`Mob.Ble.MobileBridge` + `Mob.Routing.BLE`).
       Normalizes via `BLE.Adapter.event_message/1` (or direct mapping)
       and logs under the `MobBleSelfTest` tag.
  ...
  def handle_info(:heartbeat, state) do
    ...
    rt_probe(:wake, :selftest_heartbeat, %{
      ...
      mob_peers: MapSet.size(state.mob_peers),
      sent: state.sent,
  ...
  def handle_info({source, :bridge_event, raw}, state)
      when source in [Mob.Node.NativeBridge, Mob.Ble.MobileBridge, __MODULE__] do
    ...
    state =
      case decoded do
        {Adapter, :event, %Mob.Node.BLE.Events.DeviceDiscovered{} = e} ->
          maybe_log_mob_peer(e.device_id, e.rssi, e.advertisement, state)

        {Adapter, :event, %Mob.Node.BLE.Events.AdvertisementReceived{} = e} ->
          maybe_log_mob_peer(e.device_id, e.rssi, e.advertisement, state)

        {Adapter, :event, %Mob.Node.BLE.Events.ReceivedMessage{} = e} ->
        ...
        {Adapter, :event, %Mob.Node.BLE.Events.ReceivedMessageBeacon{} = e} ->
        ...
        {Adapter, :event, %Mob.Node.BLE.Events.Error{} = e} ->
  ...
  # Internal.BridgeProtocol -> Mob.Routing.BLE). When the legacy
  ...
  # common ones to our counters and mob-peer logging so the heavy
  ...
  def handle_info({:ble_peer_up, device_id, metadata}, state) do
    ...
    state = maybe_log_mob_peer(device_id, rssi, advert, state)
  ...
  # Mob.Routing.BLE forwards bridge frame events as
  # `{:mob_routing, :ble, {:frame, peer_id, frame}}`. When `App` wires
  ...
  def handle_info({:mob_routing, :ble, {:frame, peer_id, frame}}, state)
      when is_binary(frame) do
    state = %{state | event_count: state.event_count + 1}

    case Mob.Node.BLE.MessageEnvelope.parse(frame) do
      {:ok, envelope} ->
        received = %Mob.Node.BLE.Events.ReceivedMessage{
        ...
        Mob.Node.BLE.Observability.record(received)
  ...
  def handle_info({:mob_routing, :ble, {:peer_up, _peer}}, state) do
    {:noreply, %{state | event_count: state.event_count + 1}}
  end

  def handle_info({:mob_routing, :ble, {:peer_down, _peer_id}}, state) do
    {:noreply, %{state | event_count: state.event_count + 1}}
  end
  ...
  # A MeshX peer tablet advertises its adapter name ("mob-<suffix>");
  ...
  defp maybe_log_mob_peer(device_id, rssi, advertisement, state) do
    ...
    # before scanning for the peer's "mob-*" manufacturer-data tag.
    ...
    if mob_advertisement?(bytes) and
      ...
      Map.update!(state, :mob_peers, &MapSet.put(&1, device_id))
    else
      state
    end
  end
  ...
  defp mob_advertisement?(bytes) do
    bytes
    |> :binary.bin_to_list()
    |> printable_runs()
    |> Enum.any?(&String.contains?(&1, "mob"))
  end
  ...
  defp extract_name(bytes) do
    ...
    |> :binary.bin_to_list()
    |> printable_runs()
    |> Enum.find("?", &String.contains?(&1, "mob"))
  end
