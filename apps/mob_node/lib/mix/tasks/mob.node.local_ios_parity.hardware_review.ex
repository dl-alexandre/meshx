defmodule Mix.Tasks.Mob.Node.LocalIosParity.HardwareReview do
  @moduledoc """
  Reviews iOS advert-only hardware evidence metadata.

      mix mob.node.local_ios_parity.hardware_review
      mix mob.node.local_ios_parity.hardware_review --template --out artifacts/local-ble/run/ios/evidence.json
      mix mob.node.local_ios_parity.hardware_review --input artifacts/local-ble/run/ios/evidence.json
      mix mob.node.local_ios_parity.hardware_review --input artifacts/local-ble/run/ios/evidence.json --json --out tmp/local-ios-parity-hardware-review.json
  ...
  defp maybe_print_template_hint(_review) do
    Mix.shell().info(
      "IOS_PARITY_HARDWARE_TEMPLATE command=mix mob.node.local_ios_parity.hardware_review --template --out artifacts/local-ble/<run-id>/ios/evidence.json"
    )
  end
end
