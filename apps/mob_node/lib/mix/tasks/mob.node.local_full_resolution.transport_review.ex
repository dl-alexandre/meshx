defmodule Mix.Tasks.Mob.Node.LocalFullResolution.TransportReview do
  @moduledoc """
  Reviews full-message-resolution transport evidence metadata.

      mix mob.node.local_full_resolution.transport_review
      mix mob.node.local_full_resolution.transport_review --template --out artifacts/local-ble/run/full-resolution/evidence.json
      mix mob.node.local_full_resolution.transport_review --input artifacts/local-ble/run/full-resolution/evidence.json
      mix mob.node.local_full_resolution.transport_review --input artifacts/local-ble/run/full-resolution/evidence.json --json --out tmp/local-full-resolution-transport-review.json
  ...
  defp maybe_print_template_hint(_review) do
    Mix.shell().info(
      "FULL_RESOLUTION_TRANSPORT_TEMPLATE command=mix mob.node.local_full_resolution.transport_review --template --out artifacts/local-ble/<run-id>/full-resolution/evidence.json"
    )
  end
end
