defmodule Mix.Tasks.Mob.Node.LocalKnownGoodTransport.Review do
  @moduledoc """
  Reviews known-good constrained fetch transport evidence metadata.

      mix mob.node.local_known_good_transport.review
      mix mob.node.local_known_good_transport.review --template --out artifacts/local-ble/run/transport/evidence.json
      mix mob.node.local_known_good_transport.review --input artifacts/local-ble/run/transport/evidence.json
      mix mob.node.local_known_good_transport.review --input artifacts/local-ble/run/transport/evidence.json --json --out tmp/local-known-good-transport-review.json
  ...
  defp maybe_print_template_hint(_review) do
    Mix.shell().info(
      "KNOWN_GOOD_TRANSPORT_TEMPLATE command=mix mob.node.local_known_good_transport.review --template --out artifacts/local-ble/<run-id>/transport/evidence.json"
    )
  end
end
