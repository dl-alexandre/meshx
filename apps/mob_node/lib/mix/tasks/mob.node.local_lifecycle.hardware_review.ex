defmodule Mix.Tasks.Mob.Node.LocalLifecycle.HardwareReview do
  @moduledoc """
  Reviews mobile BLE lifecycle hardware evidence metadata.

      mix mob.node.local_lifecycle.hardware_review
      mix mob.node.local_lifecycle.hardware_review --template --out artifacts/local-ble/run/lifecycle/evidence.json
      mix mob.node.local_lifecycle.hardware_review --input artifacts/local-ble/run/lifecycle/evidence.json
      mix mob.node.local_lifecycle.hardware_review --input artifacts/local-ble/run/lifecycle/evidence.json --json --out tmp/local-lifecycle-hardware-review.json
  ...
  alias Mob.Node.BLE.LocalLifecycleHardwareEvidenceReview
  ...
  defp maybe_print_template_hint(_review) do
    Mix.shell().info(
      "LIFECYCLE_HARDWARE_TEMPLATE command=mix mob.node.local_lifecycle.hardware_review --template --out artifacts/local-ble/<run-id>/lifecycle/evidence.json"
    )
  end
end
