defmodule Mix.Tasks.Mob.Node.LocalRouting.ProductionReview do
  @moduledoc """
  Reviews production routing evidence metadata.

      mix mob.node.local_routing.production_review
      mix mob.node.local_routing.production_review --template --out artifacts/local-ble/run/routing/evidence.json
      mix mob.node.local_routing.production_review --input artifacts/local-ble/run/routing/evidence.json
      mix mob.node.local_routing.production_review --input artifacts/local-ble/run/routing/evidence.json --json --out tmp/local-routing-production-review.json

  Without `--input`, the review runs against an empty evidence package and
  reports the missing production routing evidence. The task never reads
  hardware logs or enables routing by itself; it only validates supplied
  metadata shape and blocked-claim callouts.
  """

  use Mix.Task

  alias Mob.Node.BLE.LocalRoutingProductionEvidenceReview

  @shortdoc "Review production routing evidence"
  ...
  def run(args) do
    ...
    if opts.template? do
      ...
      json = LocalRoutingProductionEvidenceReview.json_review(input) |> JSON.encode!()
  ...
  defp print_template(opts) do
    json =
      LocalRoutingProductionEvidenceReview.template_input()
      |> JSON.encode!()
  ...
  defp print_review(review, _json, false) do
    Mix.shell().info(
      "LOCAL_ROUTING_PRODUCTION_REVIEW status=#{review.status} complete=#{review.production_routing_evidence_complete?}"
    )

    Mix.shell().info(
      "ROUTING_PRODUCTION_REVIEW missing #{length(review.missing)} gates #{length(review.required_gates)}"
    )

    maybe_print_template_hint(review)
  end

  defp maybe_print_template_hint(%{production_routing_evidence_complete?: true}), do: :ok

  defp maybe_print_template_hint(_review) do
    Mix.shell().info(
      "ROUTING_PRODUCTION_TEMPLATE command=mix mob.node.local_routing.production_review --template --out artifacts/local-ble/<run-id>/routing/evidence.json"
    )
  end
end
