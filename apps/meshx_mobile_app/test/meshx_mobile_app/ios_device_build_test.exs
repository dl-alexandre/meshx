defmodule MeshxMobileApp.IOSDeviceBuildTest do
  ...
  test "patches Mob physical device build with MeshX BLE bridge sources" do
    ...
    assert script =~ ~s("$MESHX_SWIFT_DIR/MeshxFetchProtocol.swift")
    assert script =~ ~s("$MESHX_SWIFT_DIR/MeshxFetchGatt.swift")
    assert script =~ ~s("$MESHX_SWIFT_DIR/MeshxFetchGattResponder.swift")
