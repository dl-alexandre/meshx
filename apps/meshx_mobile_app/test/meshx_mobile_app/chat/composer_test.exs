defmodule MeshxMobileApp.Chat.ComposerTest do
  ...
  describe "build_packet/3" do
    ...
    test "round-trips through the codec (encoded packet decodes back to the same envelope payload)" do
      ...
      {:ok, frame} = MeshxProtocol.Codec.encode_packet(packet)
      assert {:ok, decoded, <<>>} = MeshxProtocol.Codec.decode_packet(frame)
