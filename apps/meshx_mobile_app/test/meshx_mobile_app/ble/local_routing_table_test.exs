defmodule MeshxMobileApp.BLE.LocalRoutingTableTest do
  use ExUnit.Case, async: true

  alias MeshxMobileApp.BLE.{LocalRoutingTable, PeerCapabilities}
  ...
  test "entries reject anonymous stale suspicious or non-MeshX candidates" do
    ...
    stale = peer(peer_id: "meshx-stale", presence: :stale)
    suspicious = peer(peer_id: "meshx-bad", suspicious?: true, collision_count: 1)
    non_meshx = peer(peer_id: "meshx-plain", capabilities: %PeerCapabilities{})

    entries = LocalRoutingTable.entries([anonymous, stale, suspicious, non_meshx])
    ...
    assert Enum.any?(entries, &(:missing_meshx_capability in &1.blocked_reasons))
  ...
  test "missing destination is explicit and not an error" do
    selection = LocalRoutingTable.select([peer(peer_id: "meshx-alpha")], "meshx-missing")
