defmodule MeshxMobileApp.BLE.MessagePlannerTest do
  use ExUnit.Case, async: true

  alias MeshxMobileApp.BLE.{MessageEnvelope, MessagePlanner, PeerCapabilities}
  alias MeshxMobileApp.BLE.MessagePlanner.{BroadcastPlan, DirectedPlan}
  ...
  describe "plan/3 — broadcast messages" do
    ...
    test "broadcast excludes stale + expired peers" do
      ...
      peers = [
        named_capable_summary("meshx-active", last_seen_at: 99_500),
        named_capable_summary("meshx-stale", last_seen_at: 80_000),
        named_capable_summary("meshx-expired", last_seen_at: 1)
      ]

      assert {:eligible, %BroadcastPlan{candidates: c}} =
               MessagePlanner.plan(env, peers, now: 100_000)

      assert Enum.map(c, & &1.peer_id) == ["meshx-active"]
    end
