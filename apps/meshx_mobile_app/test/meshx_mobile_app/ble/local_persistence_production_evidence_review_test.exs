defmodule MeshxMobileApp.BLE.LocalPersistenceProductionEvidenceReviewTest do
    ...
    Map.new(LocalPersistenceProductionEvidenceReview.required_gates(), fn gate ->
      {Atom.to_string(gate),
       %{
         ...
         "test_command" =>
           "mix test apps/meshx_mobile_app/test/meshx_mobile_app/ble/local_persistence_#{gate}_test.exs",
         "evidence_type" =>
           LocalPersistenceProductionEvidenceReview.required_evidence_types()
           |> Map.fetch!(gate)
           |> Atom.to_string(),
