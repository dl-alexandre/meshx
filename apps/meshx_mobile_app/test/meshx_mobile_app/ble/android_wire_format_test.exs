defmodule MeshxMobileApp.BLE.AndroidWireFormatTest do
  @moduledoc """
  Contract proof: Kotlin emits the same v1 wire format that
  `MeshxMobileApp.BLE.BridgeProtocol.decode/1` understands.

  Fixtures under `test/fixtures/android_wire_v1/` mirror the exact JSON
  shape produced by `dev.meshx.mob.ble.BleEvent.toJsonObject()`. JSON
  ...
  alias MeshxMobileApp.BLE.{BridgeProtocol, Events}
  ...
  test "received_message fixture decodes to canonical ReceivedMessage" do
    ...
    assert e.raw_transport_metadata.message_payload ==
             MeshxMobileApp.BLE.MessageEnvelope.encode(e.envelope)
