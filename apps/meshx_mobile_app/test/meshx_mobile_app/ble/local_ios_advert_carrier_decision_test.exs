defmodule MeshxMobileApp.BLE.LocalIOSAdvertCarrierDecisionTest do
  ...
  test "snapshot separates iOS observe implementation from disabled gossip emission" do
    ...
    assert snapshot.current_ios_emit_carrier == :none
  ...
  test "manufacturer-data observe carrier is implemented but still hardware-unvalidated" do
    carrier =
    ...
    assert carrier.status == :implemented_unvalidated
    assert :ios_hardware_participation in carrier.blocked_claims
    assert :ios_legacy_beacon_observed in carrier.blocked_claims
    assert Enum.any?(carrier.evidence, &String.ends_with?(&1, "BLE.swift"))
  end

  test "emit carriers do not allow iOS beacon gossip claims" do
    emit_carriers =
      LocalIOSAdvertCarrierDecision.carriers()
    ...
    refute Enum.any?(emit_carriers, &(&1.status == :implemented_unvalidated))
