defmodule MeshxMobileApp.BLE.Dispatcher.AndroidTest do
  use ExUnit.Case, async: true

  alias MeshxMobileApp.BLE.AttemptLedger.Attempt
  alias MeshxMobileApp.BLE.Dispatcher.Android
  ...
  describe "dispatch/2 — fanout + ordering" do
    test "one outcome per attempt, in input order, each invoking the bridge once" do
      attempts =
        for id <- ["a", "b", "c"] do
          attempt(attempt_id: "att-#{id}", target_peer_id: "meshx-#{id}")
        end
      ...
      counting_send = fn a ->
        ...
        if a.target_peer_id == "meshx-b", do: {:error, :gatt_busy}, else: {:ok, :ack}
      end
