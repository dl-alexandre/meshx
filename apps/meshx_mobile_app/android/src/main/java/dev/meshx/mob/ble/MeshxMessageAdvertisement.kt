...
/**
 * Android-side parser for MeshX message advertisements.
 *
 ...
 */
object MeshxMessageAdvertisement {
        ...
        for (result in manufacturerEntries(advertisement)) {
            when (result) {
                ...
                is ManufacturerEntryResult.Entry -> {
                    ...
                    if (entry.companyIdentifier != BleDispatcher.MESHX_COMPANY_IDENTIFIER) {
                        continue
                    }
                    ...
                    return when (val parsed = MeshxMessageEnvelope.parse(entry.payload)) {
                        is MeshxMessageEnvelope.ParseResult.Ok -> DecodeResult.Received(
                            receivedMessage(
                                parsed.envelope,
                                entry,
                                advertisement,
                        ...
                        is MeshxMessageEnvelope.ParseResult.Error -> DecodeResult.Error(
                            BleEvent.Error(
                                kind = BleEvent.Companion.ErrorKind.UNKNOWN,
    ...
    private fun manufacturerEntries(advertisement: ByteArray): List<ManufacturerEntryResult> {
        ...
        while (offset < advertisement.size) {
            ...
            if (structureEnd > advertisement.size) {
                if (truncatedMeshxAdStructure(advertisement, structureStart)) {
    ...
    private fun truncatedMeshxAdStructure(advertisement: ByteArray, structureStart: Int): Boolean {
        ...
        val companyIdentifier =
            (advertisement[dataStart].toInt() and 0xFF) or
                ((advertisement[dataStart + 1].toInt() and 0xFF) shl 8)
        return companyIdentifier == BleDispatcher.MESHX_COMPANY_IDENTIFIER &&
