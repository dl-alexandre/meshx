 ...
 * 2. A connectable MeshxFetchGatt service advertisement tells us where
 ...
 */
interface MeshxBeaconFetchCoordinatorHook {
...
interface MeshxBeaconFetchClient {
    fun fetchOnce(deviceAddress: String, request: MeshxFetchProtocol.Request): Boolean
    fun stopClient()
}

class MeshxBeaconFetchCoordinator(
    context: Context?,
    ...
    private val fetchClientFactory: (MeshxFetchGatt.ClientListener) -> MeshxBeaconFetchClient =
        { listener -> MeshxFetchGatt(requireNotNull(context).applicationContext, adapter, listener) }
) : MeshxFetchGatt.ClientListener, MeshxBeaconFetchCoordinatorHook {
    ...
    private data class PendingFetch(
        ...
        val cue: FetchCue,
        val fetchClient: MeshxBeaconFetchClient
    )
    ...
    override fun onFetchComplete(
        ...
        val parsed = MeshxMessageEnvelope.parse(envelope) as? MeshxMessageEnvelope.ParseResult.Ok
        if (parsed == null) {
            sink.accept(
                BleEvent.Error(
    ...
    companion object {
        private const val LOGCAT_TAG = "MeshxBeaconFetch"
