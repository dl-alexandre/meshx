...
/**
 * Real-transport send bridge for one MeshX attempt.
 *
 * Mirrors the Elixir `MeshxMobileApp.BLE.Dispatcher.Android` shape:
 ...
 * outcome as a v1 wire-format JSON line tagged `MeshxBleDispatch` so
...
class BleDispatcher internal constructor(
    ...
    companion object {
        ...
        fun legacyBeaconPayload(envelope: MeshxMessageEnvelope.Decoded): ByteArray {
            ...
            return byteArrayOf(
                ...
                MeshxMessageEnvelope.CURRENT_VERSION.toByte(),
                kindCode.toByte(),
                0 // reserved flags
        ...
        fun legacyBeaconStartedJsonLine(
            attemptId: String,
            beacon: ByteArray,
            envelope: MeshxMessageEnvelope.Decoded
        ): String {
            return JSONObject().apply {
                put("v", 1)
                ...
                put("envelope_version", MeshxMessageEnvelope.CURRENT_VERSION)
                put("window_ms", ADVERTISE_WINDOW_MS)
    ...
    fun dispatch(
        ...
        val envelope = when (val parsed = MeshxMessageEnvelope.parse(payload)) {
            is MeshxMessageEnvelope.ParseResult.Ok -> parsed.envelope
            is MeshxMessageEnvelope.ParseResult.Error -> {
                return finished(
                    attemptId, messageId, targetPeerId, targetDeviceIds,
    ...
    private fun startLegacyAdvertising(
        ...
        now: Long,
        payload: ByteArray,
        legacyBeaconEnvelope: MeshxMessageEnvelope.Decoded?
    ) {
