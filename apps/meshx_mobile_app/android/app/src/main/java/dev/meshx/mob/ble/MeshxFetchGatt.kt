...
class MeshxFetchGatt(
    ...
    fun startResponder(envelope: ByteArray, responderPeerId: String): Boolean {
        ...
        val decoded = (parsed as? MeshxMessageEnvelope.ParseResult.Ok)?.envelope
        if (decoded == null) {
    ...
    private fun serverCallback(
        ...
        object : BluetoothGattServerCallback() {
            override fun onCharacteristicWriteRequest(
                ...
                val request = MeshxFetchProtocol.decodeRequest(value)
                val response = if (request == null) {
                    MeshxFetchProtocol.Response(
                        requestId = "invalid",
                        messageIdHash = ByteArray(8),
                        status = MeshxFetchProtocol.STATUS_INVALID_REQUEST,
                        envelope = null,
                ...
                log("fetch_request_received") {
                    ...
                    put("status", MeshxFetchProtocol.statusName(response.status))
                    put("responder_peer_id", responderPeerId)
                    putDeviceDiagnostics()
                }
    ...
    private fun clientCallback(request: MeshxFetchProtocol.Request): BluetoothGattCallback =
        object : BluetoothGattCallback() {
            ...
            override fun onServicesDiscovered(gatt: BluetoothGatt?, status: Int) {
                ...
                val bytes = MeshxFetchProtocol.encodeRequest(request)
    ...
    @SuppressLint("MissingPermission")
    private fun startServiceDiscovery(gatt: BluetoothGatt?, request: MeshxFetchProtocol.Request) {
        clearTimeout()
    ...
    private fun handleRead(
        ...
        val response = MeshxFetchProtocol.decodeResponse(value)
        if (response == null) {
    ...
    companion object {
        const val LOGCAT_TAG = "MeshxBleFetch"
