...
class MeshxMessageAdvertisementTest {
    ...
    @Test fun `message advertisement becomes canonical received message`() {
        ...
        val result = MeshxMessageAdvertisement.decodeScanRecord(
        ...
        assertTrue(result is MeshxMessageAdvertisement.DecodeResult.Received)
        val event = (result as MeshxMessageAdvertisement.DecodeResult.Received).event

        assertEquals(ByteArray(16) { if (it == 15) 1 else 0 }.toList(), event.messageId.toList())
        assertEquals("meshx-alpha", event.senderPeerId)
        assertEquals("meshx-beta", event.recipientPeerId)
    ...
    @Test fun `ordinary advertisements are ignored`() {
        ...
        assertEquals(MeshxMessageAdvertisement.DecodeResult.NotMessageAdvertisement, result)
    }
    ...
    @Test fun `legacy message beacon becomes canonical beacon event`() {
        ...
        assertTrue(result is MeshxMessageAdvertisement.DecodeResult.ReceivedBeacon)
        val event = (result as MeshxMessageAdvertisement.DecodeResult.ReceivedBeacon).event
    ...
    @Test fun `malformed tagged message advertisement becomes tagged error`() {
        ...
        assertTrue(result is MeshxMessageAdvertisement.DecodeResult.Error)
        val event = (result as MeshxMessageAdvertisement.DecodeResult.Error).event
