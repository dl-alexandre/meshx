 ...
 * MX envelope through `MeshxFetchGattResponder`; Android scans the cue,
 * connects to the iOS fetch service, writes MFQ, and reads MFR.
 *
 * Before running, launch the iOS MeshxMobileHarness on the peer device
 * with `--meshx-auto-beacon` so it continuously publishes responder
 * envelopes.
 */
...
class IOSResponderFetchSmokeTest {
    ...
    fun androidFetchesEnvelopeServedByIOSResponder() {
        ...
        val fetcher = MeshxFetchGatt(context, adapter)
        ...
        val callback = object : ScanCallback() {
            override fun onScanResult(callbackType: Int, result: ScanResult) {
                ...
                val advertisesFetchService =
                    record.serviceUuids?.contains(ParcelUuid(MeshxFetchGatt.SERVICE_UUID)) == true
                ...
                val senderHash = localName
                    ...
                    ?.let { MeshxFetchGatt.messageIdHash(it.toByteArray(Charsets.UTF_8)) }
        ...
        try {
            ...
            val parsed = MeshxMessageEnvelope.parse(requireNotNull(envelope))
            assertTrue("Expected fetched envelope to parse, got $parsed", parsed is MeshxMessageEnvelope.ParseResult.Ok)
        } finally {
