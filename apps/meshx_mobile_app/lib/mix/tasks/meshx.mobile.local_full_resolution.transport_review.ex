defmodule Mix.Tasks.Meshx.Mobile.LocalFullResolution.TransportReview do
  @moduledoc """
  Reviews full-message-resolution transport evidence metadata.

      mix meshx.mobile.local_full_resolution.transport_review
      mix meshx.mobile.local_full_resolution.transport_review --template --out artifacts/local-ble/run/full-resolution/evidence.json
      mix meshx.mobile.local_full_resolution.transport_review --input artifacts/local-ble/run/full-resolution/evidence.json
      mix meshx.mobile.local_full_resolution.transport_review --input artifacts/local-ble/run/full-resolution/evidence.json --json --out tmp/local-full-resolution-transport-review.json

  Without `--input`, the review runs against an empty evidence package and
  reports the missing transport evidence. The task never fetches envelopes,
  opens BLE connections, or enables resolution claims; it only validates
  ...
  @shortdoc "Review full-message-resolution transport evidence"
  ...
  def run(args) do
    ...
    if opts.template? do
      ...
      json = LocalFullMessageResolutionEvidenceReview.json_review(input) |> JSON.encode!()
  ...
  defp print_template(opts) do
    json =
      LocalFullMessageResolutionEvidenceReview.template_input()
      |> JSON.encode!()
  ...
  defp print_review(review, _json, false) do
    Mix.shell().info(
      "LOCAL_FULL_RESOLUTION_TRANSPORT_REVIEW status=#{review.status} complete=#{review.full_resolution_transport_evidence_complete?}"
    )

    Mix.shell().info(
      "FULL_RESOLUTION_TRANSPORT_REVIEW missing #{length(review.missing)} gates #{length(review.required_gates)}"
    )

    maybe_print_template_hint(review)
  end

  defp maybe_print_template_hint(%{full_resolution_transport_evidence_complete?: true}), do: :ok

  defp maybe_print_template_hint(_review) do
    Mix.shell().info(
      "FULL_RESOLUTION_TRANSPORT_TEMPLATE command=mix meshx.mobile.local_full_resolution.transport_review --template --out artifacts/local-ble/<run-id>/full-resolution/evidence.json"
    )
  end
end
