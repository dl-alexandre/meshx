defmodule Mix.Tasks.Meshx.Mobile.LocalPersistence.ProductionReview do
  @moduledoc """
  Reviews production-default local inbox persistence evidence metadata.

      mix meshx.mobile.local_persistence.production_review
      mix meshx.mobile.local_persistence.production_review --template --out artifacts/local-ble/run/persistence/evidence.json
      mix meshx.mobile.local_persistence.production_review --input artifacts/local-ble/run/persistence/evidence.json
      mix meshx.mobile.local_persistence.production_review --input artifacts/local-ble/run/persistence/evidence.json --json --out tmp/local-persistence-production-review.json
  ...
  alias MeshxMobileApp.BLE.LocalPersistenceProductionEvidenceReview
  ...
  defp maybe_print_template_hint(_review) do
    Mix.shell().info(
      "PERSISTENCE_PRODUCTION_TEMPLATE command=mix meshx.mobile.local_persistence.production_review --template --out artifacts/local-ble/<run-id>/persistence/evidence.json"
    )
  end
end
