defmodule Mix.Tasks.Meshx.Mobile.LocalRouting.ProductionReview do
  @moduledoc """
  Reviews production routing evidence metadata.

      mix meshx.mobile.local_routing.production_review
      mix meshx.mobile.local_routing.production_review --template --out artifacts/local-ble/run/routing/evidence.json
      mix meshx.mobile.local_routing.production_review --input artifacts/local-ble/run/routing/evidence.json
      mix meshx.mobile.local_routing.production_review --input artifacts/local-ble/run/routing/evidence.json --json --out tmp/local-routing-production-review.json
  ...
  defp maybe_print_template_hint(_review) do
    Mix.shell().info(
      "ROUTING_PRODUCTION_TEMPLATE command=mix meshx.mobile.local_routing.production_review --template --out artifacts/local-ble/<run-id>/routing/evidence.json"
    )
  end
end
