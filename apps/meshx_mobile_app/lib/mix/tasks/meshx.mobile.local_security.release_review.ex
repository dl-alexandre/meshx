defmodule Mix.Tasks.Meshx.Mobile.LocalSecurity.ReleaseReview do
  @moduledoc """
  Reviews local security release evidence metadata.

      mix meshx.mobile.local_security.release_review
      mix meshx.mobile.local_security.release_review --template --out artifacts/local-ble/run/security/evidence.json
      mix meshx.mobile.local_security.release_review --input artifacts/local-ble/run/security/evidence.json
      mix meshx.mobile.local_security.release_review --input artifacts/local-ble/run/security/evidence.json --json --out tmp/local-security-release-review.json
  ...
  defp maybe_print_template_hint(_review) do
    Mix.shell().info(
      "SECURITY_RELEASE_TEMPLATE command=mix meshx.mobile.local_security.release_review --template --out artifacts/local-ble/<run-id>/security/evidence.json"
    )
  end
end
