defmodule MeshxMobileApp.BLE.LocalKnownGoodTransportEvidenceReview do
  @moduledoc """
  Pure review contract for known-good constrained fetch transport metadata.

  This module validates operator-supplied evidence for the transport decision
  and standalone interop gates that must pass before any real beacon fetch path
  can be trusted. It does not open BLE connections, run GATT, fetch envelopes,
  scan, advertise, route, persist, ACK, retry, encrypt, authenticate, or enable
  full-message resolution.
  """

  alias MeshxMobileApp.BLE.{LocalFetchTransportValidationPlan, LocalHardwareValidationGates}
  ...
  @required_gate_blocked_claims %{
    ...
    standalone_interop_matrix: [:single_direction_only_interop],
    tiny_read_write_probe: [:connect_without_tiny_read_write],
    known_bad_pair_separation: [:known_bad_pair_reuse, :status_133_overclaim],
    constrained_fetch_prerequisite: [:fetch_exchange_overclaim],
  ...
  def review(input) when is_map(input) do
    ...
    %{
      ...
      known_good_transport_evidence_complete?: missing == [],
      ...
      missing: missing,
      notes: [
        "Ready evidence means the supplied metadata covers known-good transport selection and standalone interop gates.",
        "This review does not validate a fetch exchange or enable full-message resolution.",
        "SM-T577U/SM-T390 GATT status 133 remains known-bad evidence, not a known-good transport."
      ]
    }
  end
