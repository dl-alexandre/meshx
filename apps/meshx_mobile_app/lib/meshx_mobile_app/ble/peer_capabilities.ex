defmodule MeshxMobileApp.BLE.PeerCapabilities do
  @moduledoc """
  Passive parsing of MeshX peer-capability advertisements.

  Distinct from `MeshxMobileApp.BLE.Capabilities` (which describes
  *bridge* capabilities exchanged at the Kotlin/Swift ↔ Elixir
  ...
  def parse(advertisement) when is_binary(advertisement) do
    case find_meshx_record(advertisement) do
      nil -> %__MODULE__{}
  ...
  defp find_meshx_record(<<>>), do: nil
  defp find_meshx_record(<<0, _::binary>>), do: nil

  defp find_meshx_record(<<len, rest::binary>>) when byte_size(rest) < len do
    nil
  end

  defp find_meshx_record(
         <<len, @manufacturer_specific_data, data::binary-size(len - 1), tail::binary>>
       ) do
    case data do
      <<@meshx_marker, payload::binary>> -> payload
      _ -> find_meshx_record(tail)
    end
  end

  defp find_meshx_record(<<len, _type, _data::binary-size(len - 1), tail::binary>>) do
    find_meshx_record(tail)
  end
