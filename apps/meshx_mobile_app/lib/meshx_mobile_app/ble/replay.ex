defmodule MeshxMobileApp.BLE.Replay do
  @moduledoc """
  Deterministic replay of captured v1 BLE events into any process that
  understands `MeshxMobileApp.BLE.Adapter` event messages (today: `Session`).
  ...
  Android or iOS adapter uses — `MeshxMobileApp.BLE.Adapter.event_message/1`,
  which routes through `MeshxMobileApp.BLE.BridgeProtocol.decode/1`. The
  ...
  malformed lines.
  """
  @spec load!(Path.t()) :: [MeshxMobileApp.BLE.Event.t()]
  def load!(path) do
    path
    |> stream_raw()
    |> Enum.map(fn raw ->
      case MeshxMobileApp.BLE.BridgeProtocol.decode(raw) do
        {:ok, event} -> event
  ...
  process by sending canonical `MeshxMobileApp.BLE.Adapter` messages.
