defmodule MeshxMobileApp.Chat.ChannelNativeSurface do
  @moduledoc """
  Pure view-model -> screen surface translator for a chat channel.

  Takes a `MeshxMobileApp.Chat.ChannelViewModel` snapshot and produces a
  flat, render-ready struct the `ChatScreen` can iterate over without
  doing any formatting itself. Mirrors the shape pattern of
  `MeshxMobileApp.BLE.LocalInboxNativeSurface` so the same screen
