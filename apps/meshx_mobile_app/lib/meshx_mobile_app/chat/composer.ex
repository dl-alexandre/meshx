defmodule MeshxMobileApp.Chat.Composer do
  @moduledoc """
  Builds an outbound chat packet from user-authored text.

  Pure module — no GenServer, no transport coupling. The caller
  dispatches the returned `%MeshxProtocol.Packet{}` (typically via
  `MeshxRuntime.Router.broadcast_packet/2`) so ack, retry, and
  multi-hop are handled by the runtime exactly the same way as any
  other `:data` packet.

  The chat payload is a `MeshxMobileApp.BLE.MessageEnvelope` whose
  ...
  alias MeshxMobileApp.Chat.Identity
