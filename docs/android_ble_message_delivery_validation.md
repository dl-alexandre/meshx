...
| Latest ledger update | 2026-05-12 America/Los_Angeles; current full-envelope Android-to-Android rerun artifact is `/tmp/meshx-android-m26-live-current/summary.json`; current legacy-beacon rerun artifact is `/tmp/meshx-android-m26b-legacy-current/summary.json` |
...
| M26B artifact | `/tmp/meshx-android-m26b-legacy-current/summary.json`; summary reports `legacy_beacon_delivery_complete=true` with no legacy blockers |
...
cd Examples/MeshxMobileHarness
xcodegen generate
xcodebuild -project MeshxMobileHarness.xcodeproj -scheme MeshxMobileHarness -destination generic/platform=iOS -allowProvisioningUpdates DEVELOPMENT_TEAM=2MP8QWK7R6 CODE_SIGN_STYLE=Automatic build
xcrun devicectl device install app --device 39FD8D3A-9CA5-5DEF-AFC0-AA5205511117 "<DerivedData>/Build/Products/Debug-iphoneos/Meshx Mobile Harness.app"
xcrun devicectl device process launch --device 39FD8D3A-9CA5-5DEF-AFC0-AA5205511117 --terminate-existing --console dev.meshx.mobile.harness --meshx-auto-scan

cd ../..
xcrun swift build --product MeshxMessageObserverCLI
open -n -W /tmp/MeshxMessageObserver.app --args --timeout 45 --exit-after-first --log-file /tmp/meshx-mac-observer-proof.log --log-discoveries
...
adb -s R52W90AW7EN shell am start -n dev.meshx.mob/.MainActivity --ez meshx_dispatch_test true
adb -s R52W90AW7EN logcat -d -s MeshxBle:I MeshxBleDispatch:I AndroidRuntime:E
adb -s R52W90AW7EN logcat -d -s MeshxBle:I MeshxBleDispatch:I AndroidRuntime:E > /tmp/meshx-android-proof.log
ruby -rjson -e 'android = File.readlines("/tmp/meshx-android-proof.log").find { |line| line.include?("advertising_set_started") }; observer = File.readlines("/tmp/meshx-mac-observer-proof.log").find { |line| line.include?("received_message") }; android_json = JSON.parse(android[/\{.*\}/]); observer_json = JSON.parse(observer[/\{.*\}/]); puts "payload_match=#{android_json.fetch("payload") == observer_json.fetch("envelope")}"; puts "payload=#{android_json.fetch("payload")}"; puts "received_event=#{observer_json.fetch("event")}"'
...
adb -s R52W90AW7EN shell am start -n dev.meshx.mob/.MainActivity --ez meshx_dispatch_test true --ez meshx_dispatch_connectable true
```
...
creates a timestamped `/tmp/meshx-android-m26-*` directory and prints
...
`observer_m14_consistent`, `observer_meshx_transport_metadata`,
...
lines with timestamp/pid/tid/tag provenance (`MeshxBleDispatch`,
`MeshxBleControl`, and `MeshxBle`), and `sender_device` /
...
| Observed payload becomes unchanged `ReceivedMessage` | `observer_m14_consistent`, `observer_meshx_transport_metadata`, and `payload_match` must all be true |
...
```bash
adb -s <DEVICE_B_SERIAL> shell pm grant dev.meshx.mob android.permission.BLUETOOTH_SCAN
...
adb -s <DEVICE_B_SERIAL> shell am force-stop dev.meshx.mob
adb -s <DEVICE_B_SERIAL> shell am start -n dev.meshx.mob/.MainActivity --ez meshx_start_scan true
adb -s <DEVICE_B_SERIAL> logcat -d -s MeshxBle:I MeshxBleControl:I AndroidRuntime:E
```
...
mix test apps/meshx_mobile_app/test/meshx_mobile_app/ble/message_advertisement_test.exs apps/meshx_mobile_app/test/meshx_mobile_app/ble/replay_test.exs apps/meshx_mobile_app/test/meshx_mobile_app/ble/bridge_protocol_test.exs
# 54 tests, 0 failures

mix test apps/meshx_mobile_app/test/meshx_mobile_app/ble/android_wire_format_test.exs
# 4 tests, 0 failures

mix test apps/meshx_mobile_app/test/meshx_mobile_app/ble/peer_table_test.exs
# 26 tests, 0 failures
...
./gradlew --no-daemon testDebugUnitTest --tests dev.meshx.mob.ble.MeshxMessageAdvertisementTest --tests dev.meshx.mob.ble.MeshxMessageEnvelopeTest --tests dev.meshx.mob.ble.BleDispatcherTest --tests dev.meshx.mob.ble.BleScannerTest --tests dev.meshx.mob.ble.BleEventTest
BUILD SUCCESSFUL
...
```text
05-11 18:26:21.531  7670  7670 I MeshxBleDispatch: {"v":1,"event":"attempt_outcome","attempt_id":"spike-att-0","target_peer_id":"meshx-beta","kind":"dispatched","reason":null,"adapter":"ble_android","outcome_at_ms":13934660}
05-11 18:26:21.619  7670  7670 I MeshxBleDispatch: {"v":1,"event":"advertising_set_started","attempt_id":"spike-att-0","payload_size":60,"payload":"TVgBAAABAgMEBQYHCAkKCwwNDg8AAAGLz+VoAAELbWVzaHgtYWxwaGEKbWVzaHgtYmV0YQJUWAAAAmhp","tx_power":-7,"window_ms":5000,"connectable":false,"scannable":true,"data_carrier":"scan_response"}
```
...
`/tmp/meshx-android-sender-regression.log`, revalidated the same
...
the Swift harness launched with a command-line `--meshx-auto-scan`
observer mode. Console output:

```text
MeshxMessageObserver: scan_requested
MeshxMessageObserver: state CBManagerState(rawValue: 5)
MeshxMessageObserver: scan_started
```

No `MeshxMessageObserver: {"v":1,"event":"received_message",...}` line
...
** BUILD SUCCEEDED **
App installed:
bundleID: dev.meshx.mobile.harness
```
...
```text
Unable to launch dev.meshx.mobile.harness because the device was not, or could not be, unlocked.
```

After unlocking, the fresh harness launched with `--meshx-auto-scan`
...
An additional diagnostic `--meshx-log-discoveries` mode was added to the
...
`MeshxMessageObserver: {"v":1,"event":"received_message",...}` line was
observed during that dispatch window.

A quieter `--meshx-log-candidate-discoveries` diagnostic was then added
...
A macOS `.app` wrapper around `MeshxMessageObserverCLI` was launched so
...
advertisements, including a `meshx-ipad` advertiser, but it did not
report the Android extended advertisement as a discovery.

The Android dispatcher also has a diagnostic-only
`meshx_dispatch_connectable` intent extra that keeps the same 60-byte
M14 envelope but starts the extended advertising set as
connectable/non-scannable. Android accepted and started that radio
window:

```text
05-11 18:21:24.681  7254  7254 I MeshxBleDispatch: {"v":1,"event":"advertising_set_started","attempt_id":"spike-att-0","payload_size":60,"tx_power":-7,"window_ms":5000,"connectable":true,"scannable":false}
```
...
`/tmp/meshx-android-proof.log` and the macOS observer log to
`/tmp/meshx-mac-observer-proof.log`. A JSON comparison of Android's
...
started with the app's "Start scan" control or the `meshx_start_scan`
intent extra, a MeshX message scan record is logged directly as
canonical `received_message` JSON on `MeshxBle`; malformed tagged
...
```text
MeshxMessageAdvertisementTest
BleEventTest
MeshxMessageEnvelopeTest
```
...
| --- | --- | --- |
| Recognize MeshX message advertisements | Done offline | Elixir `MessageAdvertisement.decode/1`; Android `MeshxMessageAdvertisement.decodeScanRecord(...)`; bridge/Kotlin tests |
| Parse only M14 `MessageEnvelope` | Done offline | Elixir parser delegates to `MessageEnvelope.parse/1`; Android scanner uses `MeshxMessageEnvelope.parse(...)`; Kotlin validates M14 before advertising |
...
| Canonical `ReceivedMessage` event struct | Done offline | `MeshxMobileApp.BLE.Events.ReceivedMessage` |
...
| Supporting observer scans and logs received advertisement | Supporting hardware proof only | macOS `MeshxMessageObserverCLI` app wrapper logged the MeshX service UUID, manufacturer data length 62, and canonical `received_message`; M26 still requires the same observer proof from a second Android `adb logcat` stream |
...
| --- | --- | --- |
| M23 receive parser | Elixir `MessageAdvertisement.decode/1`; Android `MeshxMessageAdvertisement.decodeScanRecord(...)`; Swift `MessageAdvertisement.decode(...)` | Covers MeshX manufacturer-data recognition, M14-only parsing, ignored ordinary adverts, and tagged malformed errors |
...
```text
05-12 03:33:59.439 21049 21049 I MeshxBleControl: {"v":1,"event":"scan_start_result","accepted":true}
```

Latest single-device sender regression:

```text
05-12 03:28:51.033 20628 20628 I MeshxBleDispatch: {"v":1,"event":"attempt_outcome","attempt_id":"spike-att-0","message_id":"AAECAwQFBgcICQoLDA0ODw==","target_peer_id":"meshx-beta","target_device_ids":["AA:BB:CC:DD:EE:FF"],"kind":"dispatched","reason":null,"adapter":"ble_android","outcome_at_ms":46484156}
05-12 03:28:51.123 20628 20628 I MeshxBleDispatch: {"v":1,"event":"advertising_set_started","attempt_id":"spike-att-0","payload_size":60,"payload":"TVgBAAABAgMEBQYHCAkKCwwNDg8AAAGLz+VoAAELbWVzaHgtYWxwaGEKbWVzaHgtYmV0YQJUWAAAAmhp","tx_power":-7,"window_ms":5000,"connectable":false,"scannable":true,"data_carrier":"scan_response"}
```

The refreshed sender log is saved at
`/tmp/meshx-android-sender-regression-latest.log`; the refreshed
observer-readiness log is saved at
`/tmp/meshx-android-observer-readiness-latest.log`. A refreshed
verify-only cross-check against the saved macOS observer proof wrote
`/tmp/meshx-verify-latest-sender-mac-observer-summary.json` and
`/tmp/meshx-verify-latest-sender-mac-observer-summary.md` with
...
`observer_meshx_transport_metadata=true`. The same summary keeps
...
```text
scripts/android_ble_message_delivery_two_device.sh --preflight-only --wait-for-devices 30 --out-dir /tmp/meshx-android-m26-readiness-current
...
preflight_summary=/tmp/meshx-android-m26-readiness-current/summary.json
preflight_summary_markdown=/tmp/meshx-android-m26-readiness-current/summary.md
preflight_adb_mdns_service_count=0
preflight_host_usb_android_candidate_count=1

scripts/audit_android_ble_message_delivery_completion.sh /tmp/meshx-android-m26-readiness-current/summary.json
m26_complete=false
reason=m26_android_to_android_complete is not true
blockers=expected exactly two attached adb devices, found 1
summary=/tmp/meshx-android-m26-readiness-current/summary.json
summary_markdown=/tmp/meshx-android-m26-readiness-current/summary.md
adb_devices_log=/tmp/meshx-android-m26-readiness-current/adb-devices.txt
adb_mdns_log=/tmp/meshx-android-m26-readiness-current/adb-mdns-services.txt
host_usb_log=/tmp/meshx-android-m26-readiness-current/host-usb.txt
...
`/tmp/meshx-android-m26-readiness-current/summary.md` also records the
...
mix test apps/meshx_mobile_app/test/meshx_mobile_app/ble/message_advertisement_test.exs apps/meshx_mobile_app/test/meshx_mobile_app/ble/android_wire_format_test.exs apps/meshx_mobile_app/test/meshx_mobile_app/ble/bridge_protocol_test.exs apps/meshx_mobile_app/test/meshx_mobile_app/ble/replay_test.exs apps/meshx_mobile_app/test/meshx_mobile_app/ble/peer_table_test.exs
# 84 tests, 0 failures

cd apps/meshx_mobile_app/android && ./gradlew --no-daemon test
# BUILD SUCCESSFUL

cd apps/meshx_mobile_app/android && ./gradlew --no-daemon testDebugUnitTest \
  --tests 'dev.meshx.mob.ble.MeshxMessageAdvertisementTest' \
  --tests 'dev.meshx.mob.ble.MeshxMessageEnvelopeTest' \
  --tests 'dev.meshx.mob.ble.BleDispatcherTest' \
  --tests 'dev.meshx.mob.ble.BleScannerTest' \
  --tests 'dev.meshx.mob.ble.BleEventTest'
...
`/tmp/meshx-android-m26-same-device-preflight/summary.json` and
`/tmp/meshx-android-m26-same-device-preflight/summary.md`. The current
...
adb -s R52W90AW7EN shell pm list packages dev.meshx.mob
adb -s R52W90AW7EN shell pm list features
adb -s R52W90AW7EN shell dumpsys package dev.meshx.mob
# package dev.meshx.mob is installed; android.hardware.bluetooth_le is present
