...
>
> # at boot (see Mob.Node.App.maybe_start_mob_ble_transport/0)
> {:ok, _} =
...
>
> > The rest of this document describes the `Mob.Routing.BLE` adapter
> > and `Mob.Routing.BLE.Bridge` contract in detail. Read it when writing
...
`mob_routing_ble` is the supported integration path for platform BLE code.
...
* `Mob.Routing.BLE.BluezBridge` for Linux/BlueZ. It supervises
  `priv/bin/mob_bluez_bridge`, which talks to BlueZ over D-Bus, registers the
...
{:ok, ble} =
  Mob.Routing.BLE.start_link(
    ...
    bridge_opts: [
      ...
      platform: Mob.Node.Platform.new(os: :ios, permissions: [:bluetooth])
    ]
  )

:ok = Mob.Runtime.Router.attach_transport(:ble, Mob.Routing.BLE, ble)
```
...
{:ok, ble} =
  Mob.Routing.BLE.start_link(
    event_target: Mob.Runtime.Router,
    bridge: Mob.Routing.BLE.BluezBridge,
    bridge_opts: [
      adapter: "hci0",
      local_name: "mob-relay-1",
      mtu: 185
    ]
  )
```
...
```bash
python3 apps/mob_routing_ble/priv/bin/mob_bluez_bridge --adapter hci0 --health-check
```

Release code can run the same check through the Elixir bridge module:

```elixir
Mob.Routing.BLE.BluezBridge.health_check(adapter: "hci0")
```
...
Native bridge modules implement `Mob.Routing.BLE.Bridge`:
...
send callbacks back to `Mob.Routing.BLE`.
...
Advertise transport and mobile constraints with `Mob.Routing.Capabilities`
and `Mob.Node.Platform`:

```elixir
metadata =
  %{}
  |> Map.merge(Mob.Routing.Capabilities.to_metadata(
    Mob.Routing.Capabilities.new(mtu: 185, relay: false, background_mode: :background)
  ))
  |> Map.merge(Mob.Node.Platform.to_metadata(
    Mob.Node.Platform.new(os: :ios, background_mode: :background, permissions: [:bluetooth])
  ))
```
...
`Mob.Routing.BLE.BluezBridge` covers these responsibilities for Linux/BlueZ.
...
`Mob.Routing.BLE.NoopBridge` remains the desktop/test fallback. It starts the
...
| --- | --- | --- | --- |
| `start_scan/1` returns `:ok` | ✅ | ✅ | NIF surface is identical (`src/mob_ble_nif.erl`). |
| `start_advertising/2` returns `:ok` | ✅ | ✅ | Same NIF surface. |
| Receive `%DeviceDiscovered{}` | ✅ | ❌ | iOS emits legacy `{:connected, peer_id}` / `{:disconnected, peer_id}` tuples (GATT path) — there's no equivalent "I saw an ambient BLE device" event. Adding it means surfacing `CBCentralManager` discovery results into a `mob_ble_emit_device_discovered` NIF entry. |
| Receive `%AdvertisementReceived{}` | ✅ | ❌ | Same reasoning as `DeviceDiscovered`. |
| Receive `%ReceivedMessageBeacon{}` (22-byte `MB` legacy beacon) | ✅ | ✅ | App bridge: `MobBLEClient.mobDidObserveLegacyBeacon` → `mob_ble_emit_received_message_beacon` → v1 atom-keyed map. Harness (post `fb5afa8`): `MessageAdvertisementObserver` falls through to `MobLegacyBeaconAdvertisement.parse` after MX decode declines, surfacing `mobMessageObserverDidObserveLegacyBeacon`. Validated cross-platform on hardware (Android SM-T577U → iPhone 13, see `artifacts/local-ble/2026-05-15-iphone13-sm-t577u/`). |
| Receive `%ReceivedMessage{}` (full `MX` envelope, extended advertising) | ✅ | iOS bridge present; PHY-blocked | Bridge integration done end-to-end: `MobNativeBLEBridge` instantiates `MessageAdvertisementObserver`, `mobDidObserveReceivedMessage` calls `mob_ble_emit_received_message`, NIF emits the v1 `received_message` map, `BridgeProtocol.decode/1` parses the envelope. `dev.mob.mob-central-…` confirmed registered with `bluetoothd` on hardware. **However:** iPhone 13 / iOS 26.4 and iPad12,1 / iPadOS 26.5 do not deliver non-Apple extended-advertising (AUX_ADV_IND) packets to `CBCentralManager.didDiscover`, regardless of `withServices` filter, `AllowDuplicates`, or whether Android puts the data in scan-response or primary AUX. Hardware evidence includes the 2026-05-15 iPhone 13 run and the 2026-05-17 SM-T577U -> iPad12,1 probe in `artifacts/local-ble/2026-05-17-sm-t577u-ipad9/hardware/android-aux-full-mx-ios-observe/`: Android emitted an 80-byte `MX` scan-response advert, while iOS logged MB beacons but zero direct `received_message` / `FF FF 4D 58` callback evidence. This is an iOS CoreBluetooth API limitation, not a bridge defect. Full MX envelopes on iOS arrive via the existing MB legacy-beacon + GATT-fetch path. |
| Dispatch send via legacy-beacon advertise | ✅ | ✅ | Android: `BleDispatcher.dispatch(payload, forceLegacyBeacon: true)` builds a `MobMessageEnvelope`, derives the beacon, advertises via manufacturer data. iOS: `MobBLEPeripheral.advertiseLegacyBeacon(messageId:senderPeerId:payloadKind:)` puts the 22-byte beacon on air via `CBAdvertisementDataManufacturerDataKey` with company id `0xFFFF`; bridge exposes this through `MobNativeBLEBridge.sendPing` fallback when no GATT peer is paired (commit `022b6f4`). Harness exercises the same path under `--mob-auto-beacon`. Hardware-validated on iPhone 13 (commit `fb5afa8` evidence bundle). |
...
| Adapter-state recovery (auto-replay intent) | ✅ | n/a | Android uses `BluetoothAdapter.ACTION_STATE_CHANGED` + intent replay. iOS BLE state restoration is a separate CoreBluetooth mechanism (`CBCentralManagerOptionRestoreIdentifierKey`); the equivalent is opt-in via `MobBLEClient`'s manager init options. |
...
1. ~~**Advert-only send path.**~~ **Closed.**
   `MobBLEPeripheral.advertiseLegacyBeacon(messageId:senderPeerId:payloadKind:)`
   builds the 22-byte beacon and advertises via
   `CBAdvertisementDataManufacturerDataKey` with company id `0xFFFF`.
   `MobNativeBLEBridge.sendPing` falls back to this path when no
   GATT peer is paired, mirroring Android's
   `MobBleNative.sendToPeer(forceLegacyBeacon: true)`. Validated on
   iPhone 13 hardware in commit `fb5afa8`'s evidence bundle.
2. **Full-envelope (`MX` magic) advert decode on the central side.**
   **Wired, PHY-blocked on tested iOS hardware.** `MobNativeBLEBridge`
   now owns a `MessageAdvertisementObserver` alongside `MobBLEClient`,
   started/stopped on the same lifecycle. Its
   `mobDidObserveReceivedMessage` callback forwards the
   `ReceivedMessageEvent` through the new
   `mob_ble_emit_received_message` NIF entry, which builds the v1
...
|---|---|---|
| MB legacy beacon (22 byte) | ✅ Working | `MessageAdvertisementObserver` decodes via `MobLegacyBeaconAdvertisement.parse`; emits `received_message_beacon` v1 map through `mob_ble_emit_received_message_beacon`. Hardware proof: commit `fb5afa8` evidence bundle (iPhone 13). |
| Full MX envelope receive (advert direct) | ⚠ Wired, PHY-blocked on iOS | All Swift + NIF + Elixir pieces ship in the production binary (`_mob_ble_nif_nif_init` registered in `erts_static_nif_tab`, MobNativeBLEBridge hosts `MessageAdvertisementObserver`, `mob_ble_emit_received_message` emits to the bridge). However iOS 26.4 / iPhone 13 + iPad12,1 / Broadcom BCM4387 does not deliver AUX_ADV_IND data from non-Apple `AdvertisingSet` broadcasts to `CBCentralManager.didDiscover`, regardless of scan filter or advertising-set parameter tuning. See "Extended-advertising AUX delivery limitation" below. |
| MB beacon → GATT fetch | ✅ Working | New in this session. `MessageAdvertisementObserver.rememberBeacon` caches recent MB beacons by `messageIdHash`; when a connectable advert for the MeshX fetch service UUID arrives (from a different private random address per Android `AdvertisingSet` semantics), `maybeStartFetchOnFetchService` instantiates `MobFetchGattClient`, which performs the MFQ Request / MFR Response cycle and synthesizes a `ReceivedMessageEvent` with `transport: "ble_ios_gatt_fetch"`. Hardware-validated on iPad12,1 ↔ Samsung SM-T577U: bluetoothd shows `Le topology – role: central, state: CONNECTED` to the fetch responder's MAC; Android responder logs `fetch_request_received {status: ok}` for the matched hash; iPad disconnects immediately after the read response (visible in bluetoothd) — the only path that triggers `central.cancelPeripheralConnection` in `MobFetchGattClient.finish(envelope:)`. |

### Android `MobBleNative` send surface
...
| `sendFullMxEnvelope(peerId, payload)` | Dev/test opt-in. Kotlin-only (not on JNI surface yet). Exercised by `MXFullEnvelopeSmokeTest`. | Starts a `MobFetchGatt` responder serving the full envelope + dispatches an MB legacy beacon as the cue. iOS peers running the matching `MessageAdvertisementObserver` + fetch-client see the beacon → connect to the responder → pull the full envelope via the MFQ/MFR GATT protocol. |
...
| Same as above | `withServices: [MobBLEUUID.service]` | No MX delivery |
...
The iOS harness supports `--mob-log-raw-advert-data` (use together with `--mob-auto-scan --mob-log-candidate-discoveries`) to emit `raw_advert keys=[...]` + value types for every `didDiscover`; this surfaces exactly which advertisementData fields (service UUIDs, service data, local name, TX power, etc.) from Android extended sets reach CoreBluetooth on the tested stack, guiding any future pivot to non-manufacturer-data carriers for direct cues.
...
* iPhone 13 (DairyPhoneDeaux, ECID `00008110-0006619A2132801E`, iOS
  ...
  cadence via the harness's `--mob-auto-beacon` driver.
* The same iPhone, scanning simultaneously, decoded MB legacy beacons
  ...
  `MobBleDispatch.legacy_beacon_advertising_started`) and an
